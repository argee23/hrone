...
class Reports_personnel_transactions_model extends CI_Model{
	...
	public function crystal_report()
	{
		...
		$this->db->where(array('a.type'=>'section_manager','a.added_by'=>$this->session->userdata('employee_id'),'option'=>'transaction'));
	...
	public function crystal_report_transaction($id)
	{
		...
		$this->db->join('transaction_file_maintenance b','b.id=a.transaction_id');
		$this->db->where(array('a.type'=>'section_manager','a.added_by'=>$this->session->userdata('employee_id'),'a.transaction_id'=>$id,'a.option'=>'transaction','a.InActive'=>0));
		$query = $this->db->get('crystal_report_form_transaction a');
		return $query->result();
	}
	...
	public function save_crystal_report()
	{
		...
		if($this->db->affected_rows() > 0)
		{
			...
			$this->db->where(array('added_by'=>$sm,'title'=>$title,'description'=>$desc,'type'=>'section_manager','option'=>'transaction'));
			$idd = $this->db->get('crystal_report_form_transaction');
			$p_id = $idd->row('id');

			$crystal_report_list = $this->reports_personnel_transactions_model->crystal_report_list($id);
	...
	public function generate_report_result($table_name)
	{
		...
		$where = "date(a.date_created) between '" .$from. "' and '" .$to. "'";
		$this->db->select('a.status as form_status,a.*,c.*,d.company_name,e.division_name,f.dept_name,g.section_name,h.subsection_name,i.classification as classification_name,j.location_name,k.employment_name');
		...
		$this->db->where(array('c.company_id'=>$company));
		$this->db->where($where);
	...
	{	
		...
		$this->db->join('employment k','k.employment_id=c.employment');
		$this->db->where($where);
		if($department=='All' || empty($department)){} else{ $this->db->where($department); }
		if($section=='All' || empty($section)){} else{ $this->db->where($section); }
		if($location=='All' || empty($location)){} else{ $this->db->where($location); }
		$this->db->where($subsection);
		if($employment=='All' || empty($employment)){} else{ $this->db->where('c.employment',$employment); }
		if($classification=='All' || empty($classification)){} else{ $this->db->where('c.classification',$classification); }
		if($status!='All'){ $this->db->where('a.status',$status);  } 
		$query = $this->db->get($table_name.' a');
