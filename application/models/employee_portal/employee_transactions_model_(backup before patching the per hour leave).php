...
class Employee_transactions_model extends CI_Model{
	...
	public function leave_details()
	{
		$me = $this->session->userdata('employee_id');
		$year = date('Y');
		$this->db->select('is_system_default,leave_type,available,leave_used_with_pay,year,employee_id,leave_type_id,leave_type.cutoff');
...
//new function added by mila
	...
	public function get_leave_pending($employee_id,$setting)
	{
		...
		$this->db->where('employee_leave.doc_no NOT IN (select cancelled_doc_no from employee_leave_cancel)',NULL,FALSE);
