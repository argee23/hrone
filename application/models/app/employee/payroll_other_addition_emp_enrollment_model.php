...
class Payroll_other_addition_emp_enrollment_model extends CI_Model{
...
function save_addition_enrollment(){  
		...
		$this->db->select("employee_id,other_addition_id,payroll_period_id,amount"); 
		$other_addition_enrollment = $this->db->get("other_addition_enrollment");
		if($other_addition_enrollment->num_rows()>0){
			foreach($other_addition_enrollment->result() as $data){
		...
		if($employee_info->num_rows()>0){
			foreach($employee_info->result() as $data){
					...
					if($this->input->post('coa'.$data->employee_id)==1){
						foreach($arr_oa as $rs_oa){
					
						

						if(isset($rs_oa) && $this->input->post('oa'.$rs_oa.'-'.$data->employee_id)>0 ){
							
							if(!empty($arr_exist)){
									if($arr_exist >0){//if not empty
									$this->db->delete("other_addition_enrollment", array('other_addition_id' => $rs_oa, 'employee_id'=>$data->employee_id,'payroll_period_id' => $this->input->post('pay_period')));
							...
							$this->data = array(
								...
								$this->db->delete("other_addition_enrollment", array('other_addition_id' => $rs_oa, 'employee_id'=>$data->employee_id));
