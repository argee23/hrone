...
class Application_form_model extends CI_Model{
	...
	public function search_a_job_main($criteria,$category, $province,$city,$specialization,$rangefrom,$rangeto,$criteria)
	{	
		$where = "a.salary between " .$salary_from. " and " .$salary_to. "";

		$this->db->join('system_parameters b','b.param_id=a.specialization_id');
		$this->db->where("(`".'a'.$criteria."` LIKE '%$category%')");
	...
	public function search_a_job_location($criteria,$category,$province,$city)
	{
		$this->db->where("(`".$category."` LIKE '%$criteria%')");
		if($province=='All' || $province=='not_included'){} else { $this->db->where('loc_province',$province); }
		if($city=='All' || $city=='not_included'){} else{ $this->db->where('loc_city',$city); }
	...
	public function update_priority_choice($id)
	{
		$abroad = $this->input->post('abroad');
		$local = $this->input->post('local');
		if(empty($abroad)){ $a=0; } else { $a=1; }
		if(empty($local)){ $l=0;  } else {  $l=1; }

		$this->db->where('applicant_id',$id);
		$this->db->delete('emp_priority_choice_applicant');

		$this->db->insert('emp_priority_choice_applicant',array('applicant_id'=>$id,'abroad'=>$a,'local'=>$l,'date_created'=>date('Y-m-d H:i:s')));
	}

	public function check_priority_choice()
	{
		$id = $this->session->userdata('employee_id');
		$this->db->where('applicant_id',$id);
		$query = $this->db->get('emp_priority_choice_applicant');
		return $query->row();
	}
