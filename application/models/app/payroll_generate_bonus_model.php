...
class Payroll_generate_bonus_model extends CI_Model{
...
and od_id='".$od_id."' AND ($final_payroll_coverage) ");
	...
	public function post_bonus($company_id,$employee_id,$pay_period,$gross_bonus,$tax_percentage,$bonus_tax,$final_bonus,$tdt,$tc){
		...
		$this->db->insert("payslip_bonus",$data);
	}
	...
	public function check_posted_payroll_period($id){
		...
		$query=$this->db->query("select release_payroll_period from payslip_bonus where release_payroll_period='".$id."' GROUP BY release_payroll_period " );
		return $query->row();		
	}
	...
	if($selected_payroll_option=="print_payslip" OR $selected_payroll_option=="reset_payslip"){// posted 13th month
			...
			$posted_payslip_join="inner join payslip_bonus ppt on(ei.employee_id=ppt.employee_id)"; //ppt means posted payslip table
			$posted_payslip_added_condition="and ppt.release_payroll_period='".$id."' "; //ppt means posted payslip table
			$posted_payslip_data="ppt.gross_bonus,ppt.bonus_tax,ppt.final_bonus,ppt.tax_percentage,ppt.tax_computation_type,ppt.tax_computation,ppt.employee_acknowledge,ppt.added_type,ppt.date_added,ppt.release_payroll_period,";
...
// 	//	echo "select salary_information.*,salary_rates.salary_rate_name from salary_information INNER JOIN salary_rates on (salary_information.salary_rate=salary_rates.salary_rate_id) where employee_id = '".$employee_id."' and date_effective<='".$pay_period_from."' order by date_effective desc limit 1;" ;
