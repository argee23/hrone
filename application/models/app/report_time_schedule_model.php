<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed'); 

class report_time_schedule_model extends CI_Model{
	
	public function __construct(){
		parent::__construct();	
	}

	public function get_single_report($company,$division,$department,$section,$subsection,$location,$classification,$employment,$status,$yy,$mm,$dd,$type,$date_from,$date_to,$payroll_period,$report_area,$covered_month_from,$covered_month_to,$covered_year,$groupings_type)
	{


		$month= 'working_schedule_'.$mm;
		if($yy!='All' AND $mm!='All' AND $dd!='All')
		{
			$covered_date = $yy.'-'.$mm.'-'.$dd;
		}
		else
		{
			$covered_date='';
		}	

		
		$locationf = str_replace("ll","",$location);
		$classificationf = str_replace("cc","",$classification);
		$employmentf = str_replace("ee","",$employment);
		
		$location_final = $this->get_condition($locationf,'b.location');
		$classification_final = $this->get_condition($classificationf,'b.classification');
		$employment_final = $this->get_condition($employmentf,'b.employment');

		$this->db->join('employee_info b','b.employee_id=a.employee_id');
		$this->db->join('company_info c','c.company_id=b.company_id');
		$this->db->join('division d','d.division_id=b.division_id','left');
		$this->db->join('department e','e.department_id=b.department');
		$this->db->join('section f','f.section_id=b.section');
		$this->db->join('subsection g','g.subsection_id=b.subsection','left');
		$this->db->join('classification h','h.classification_id=b.classification');
		$this->db->join('location i','i.location_id=b.location');
		$this->db->join('employment j','j.employment_id=b.employment');

		$this->db->where($location_final);
		$this->db->where($employment_final);
		$this->db->where($classification_final);

		if($covered_date!='')
		{
			$this->db->where('a.date BETWEEN "'. date('Y-m-d', strtotime($covered_date)). '" and "'. date('Y-m-d', strtotime($covered_date)).'"');
		}
		else
		{
			if($dd!='')
			{
				$this->db->where('DAY(a.date)',$dd);
			}
			if($yy!='All')
			{
				$this->db->where('YEAR(a.date)',$yy);
			}	
		}

		
		if($company=='All'){} else { $this->db->where('b.company_id',$company); }
		if($division=='All'){} else { $this->db->where('b.division_id',$division); }
		if($department=='All'){} else { $this->db->where('b.department',$department); }
		if($section=='All'){} else { $this->db->where('b.section',$section); }
		if($subsection=='All'){} else { $this->db->where('b.subsection',$subsection); }
		
		$query = $this->db->get($month.' a');
		return $query->result();

	}
	
	public function get_double_report($company,$division,$department,$section,$subsection,$location,$classification,$employment,$status,$yy,$mm,$dd,$type,$date_from,$date_to,$payroll_period,$report_area,$covered_month_from,$covered_month_to,$covered_year,$groupings_type)
	{


				$month_selected = $this->get_month_selected($date_from,$date_to);
				$locationf = str_replace("ll","",$location);
				$classificationf = str_replace("cc","",$classification);
				$employmentf = str_replace("ee","",$employment);
				
				$location_final = $this->get_condition($locationf,'b.location');
				$classification_final = $this->get_condition($classificationf,'b.classification');
				$employment_final = $this->get_condition($employmentf,'b.employment');
				
				$this->db->select('aa.*,a.*,b.*,bb.*,a.manager_in_charge as plotter,e.division_name,f.dept_name,g.section_name,h.subsection_name,c.employment_name,j.location_name');
				$this->db->join('working_schedule_group_by_sec_manager a','a.id=aa.group_id');
				$this->db->join('working_schedule_group_by_sec_manager_members bb','bb.group_id=a.id');
				$this->db->join('employee_info b','b.employee_id=bb.employee_id');
				$this->db->join('employment c','c.employment_id=b.employment');
				$this->db->join('company_info d','d.company_id=b.company_id');
				$this->db->join('division e','e.division_id=b.division_id','left');
				$this->db->join('department f','f.department_id=b.department');
				$this->db->join('section g','g.section_id=b.section');
				$this->db->join('subsection h','h.subsection_id=b.subsection','left');
				$this->db->join('classification i','i.classification_id=b.classification');
				$this->db->join('location j','j.location_id=b.location');
				
				if($company=='All'){} else { $this->db->where('b.company_id',$company); }
				if($division=='All'){} else { $this->db->where('b.division_id',$division); }
				if($department=='All'){} else { $this->db->where('b.department',$department); }
				if($section=='All'){} else { $this->db->where('b.section',$section); }
				if($subsection=='All'){} else { $this->db->where('b.subsection',$subsection); }

				$this->db->where($location_final);
				$this->db->where($employment_final);
				$this->db->where($classification_final);
				$this->db->where('aa.date BETWEEN "'. date('Y-m-d', strtotime($date_from)). '" and "'. date('Y-m-d', strtotime($date_to)).'"');

				$query  = $this->db->get('working_schedules_by_group aa');
				$group_result = $query->result();

				for($i=1;$i <=12;$i++)
				{
					if($i==10 || $i==11 || $i==12){ $ii = $i; } else{ $ii= '0'.$i; }

						$daterange_selected	= substr_replace($month_selected, "", -1);
						$array =  explode('-', $daterange_selected);
						foreach($array as $c)
						{
							if($c==$ii)
							{
								$res = 'ok';
								break;
							}
							else
							{
								$res = 'not';
							}
						}

						if($res=='ok')
						{
						
								$this->db->join('employee_info b','b.employee_id=a.employee_id');
								$this->db->join('employment c','c.employment_id=b.employment');
								$this->db->join('company_info d','d.company_id=b.company_id');
								$this->db->join('division e','e.division_id=b.division_id','left');
								$this->db->join('department f','f.department_id=b.department');
								$this->db->join('section g','g.section_id=b.section');
								$this->db->join('subsection h','h.subsection_id=b.subsection','left');
								$this->db->join('classification i','i.classification_id=b.classification');
								$this->db->join('location j','j.location_id=b.location');
								$this->db->where($location_final);
								$this->db->where($employment_final);
								$this->db->where($classification_final);
								$this->db->where('a.date BETWEEN "'. date('Y-m-d', strtotime($date_from)). '" and "'. date('Y-m-d', strtotime($date_to)).'"');
								
								if($company=='All'){} else { $this->db->where('b.company_id',$company); }
								if($division=='All'){} else { $this->db->where('b.division_id',$division); }
								if($department=='All'){} else { $this->db->where('b.department',$department); }
								if($section=='All'){} else { $this->db->where('b.section',$section); }
								if($subsection=='All'){} else { $this->db->where('b.subsection',$subsection); }

								$query  = $this->db->get('working_schedule_'.$ii.' a');
								if($i==1){ $result1  = $query->result(); }
								else if($i==2){ $result2  = $query->result(); }
								else if($i==3){ $result3  = $query->result(); }
								else if($i==4){ $result4  = $query->result(); }
								else if($i==5){ $result5  = $query->result(); }
								else if($i==6){ $result6  = $query->result(); }
								else if($i==7){ $result7  = $query->result(); }
								else if($i==8){ $result8  = $query->result(); }
								else if($i==9){ $result9  = $query->result(); }
								else if($i==10){ $result10  = $query->result(); }
								else if($i==11){ $result11  = $query->result(); }
								else if($i==12){ $result12  = $query->result(); }
					}
					else
					{
								if($i==1)     {  $result1   = 	array(); 	}
								else if($i==2){  $result2   = 	array(); 	}
								else if($i==3){  $result3   =    array();   }
								else if($i==4){  $result4   =    array();   }
								else if($i==5){  $result5   =    array();   }
								else if($i==6){  $result6   =    array();   }
								else if($i==7){  $result7   =    array();   }
								else if($i==8){  $result8   =    array();   }
								else if($i==9){  $result9  	=    array();   }
								else if($i==10){ $result10  =   array();    }
								else if($i==11){ $result11  =   array();    }
								else if($i==12){ $result12  =   array();    }	
					}

				}
				
				$all_results = array_merge($result1,$result2,$result3,$result4,$result5,$result6,$result7,$result8,$result9,$result10,$result11,$result12,$group_result);
				return $all_results;

	}	

	public function get_pp_report($company,$division,$department,$section,$subsection,$location,$classification,$employment,$status,$yy,$mm,$dd,$type,$date_from,$date_to,$payroll_period,$report_area,$covered_month_from,$covered_month_to,$covered_year,$groupings_type)
	{
				$get_payroll = $this->check_payroll_period($payroll_period);
				$pfrom = $get_payroll->complete_from;
				$pto = $get_payroll->complete_to;


				$month_selected = $this->get_month_selected($pfrom,$pto);
				$locationf = str_replace("ll","",$location);
				$classificationf = str_replace("cc","",$classification);
				$employmentf = str_replace("ee","",$employment);
				
				$location_final = $this->get_condition($locationf,'b.location');
				$classification_final = $this->get_condition($classificationf,'b.classification');
				$employment_final = $this->get_condition($employmentf,'b.employment');
				
				$this->db->select('aa.*,a.*,b.*,bb.*,a.manager_in_charge as plotter,e.division_name,f.dept_name,g.section_name,h.subsection_name,c.employment_name,j.location_name');
				$this->db->join('working_schedule_group_by_sec_manager a','a.id=aa.group_id');
				$this->db->join('working_schedule_group_by_sec_manager_members bb','bb.group_id=a.id');
				$this->db->join('employee_info b','b.employee_id=bb.employee_id');
				$this->db->join('employment c','c.employment_id=b.employment');
				$this->db->join('company_info d','d.company_id=b.company_id');
				$this->db->join('division e','e.division_id=b.division_id','left');
				$this->db->join('department f','f.department_id=b.department');
				$this->db->join('section g','g.section_id=b.section');
				$this->db->join('subsection h','h.subsection_id=b.subsection','left');
				$this->db->join('classification i','i.classification_id=b.classification');
				$this->db->join('location j','j.location_id=b.location');
				
				if($company=='All'){} else { $this->db->where('b.company_id',$company); }
				if($division=='All'){} else { $this->db->where('b.division_id',$division); }
				if($department=='All'){} else { $this->db->where('b.department',$department); }
				if($section=='All'){} else { $this->db->where('b.section',$section); }
				if($subsection=='All'){} else { $this->db->where('b.subsection',$subsection); }

				$this->db->where($location_final);
				$this->db->where($employment_final);
				$this->db->where($classification_final);
				$this->db->where('aa.date BETWEEN "'. date('Y-m-d', strtotime($pfrom)). '" and "'. date('Y-m-d', strtotime($pto)).'"');

				$query  = $this->db->get('working_schedules_by_group aa');
				$group_result = $query->result();

				for($i=1;$i <=12;$i++)
				{
					if($i==10 || $i==11 || $i==12){ $ii = $i; } else{ $ii= '0'.$i; }

						$daterange_selected	= substr_replace($month_selected, "", -1);
						$array =  explode('-', $daterange_selected);
						foreach($array as $c)
						{
							if($c==$ii)
							{
								$res = 'ok';
								break;
							}
							else
							{
								$res = 'not';
							}
						}

						if($res=='ok')
						{
						
								$this->db->join('employee_info b','b.employee_id=a.employee_id');
								$this->db->join('employment c','c.employment_id=b.employment');
								$this->db->join('company_info d','d.company_id=b.company_id');
								$this->db->join('division e','e.division_id=b.division_id','left');
								$this->db->join('department f','f.department_id=b.department');
								$this->db->join('section g','g.section_id=b.section');
								$this->db->join('subsection h','h.subsection_id=b.subsection','left');
								$this->db->join('classification i','i.classification_id=b.classification');
								$this->db->join('location j','j.location_id=b.location');
								$this->db->where($location_final);
								$this->db->where($employment_final);
								$this->db->where($classification_final);
								$this->db->where('a.date BETWEEN "'. date('Y-m-d', strtotime($pfrom)). '" and "'. date('Y-m-d', strtotime($pto)).'"');
								
								if($company=='All'){} else { $this->db->where('b.company_id',$company); }
								if($division=='All'){} else { $this->db->where('b.division_id',$division); }
								if($department=='All'){} else { $this->db->where('b.department',$department); }
								if($section=='All'){} else { $this->db->where('b.section',$section); }
								if($subsection=='All'){} else { $this->db->where('b.subsection',$subsection); }

								$query  = $this->db->get('working_schedule_'.$ii.' a');
								if($i==1){ $result1  = $query->result(); }
								else if($i==2){ $result2  = $query->result(); }
								else if($i==3){ $result3  = $query->result(); }
								else if($i==4){ $result4  = $query->result(); }
								else if($i==5){ $result5  = $query->result(); }
								else if($i==6){ $result6  = $query->result(); }
								else if($i==7){ $result7  = $query->result(); }
								else if($i==8){ $result8  = $query->result(); }
								else if($i==9){ $result9  = $query->result(); }
								else if($i==10){ $result10  = $query->result(); }
								else if($i==11){ $result11  = $query->result(); }
								else if($i==12){ $result12  = $query->result(); }
					}
					else
					{
								if($i==1)     {  $result1   = 	array(); 	}
								else if($i==2){  $result2   = 	array(); 	}
								else if($i==3){  $result3   =    array();   }
								else if($i==4){  $result4   =    array();   }
								else if($i==5){  $result5   =    array();   }
								else if($i==6){  $result6   =    array();   }
								else if($i==7){  $result7   =    array();   }
								else if($i==8){  $result8   =    array();   }
								else if($i==9){  $result9  	=    array();   }
								else if($i==10){ $result10  =   array();    }
								else if($i==11){ $result11  =   array();    }
								else if($i==12){ $result12  =   array();    }	
					}

				}
				
				$all_results = array_merge($result1,$result2,$result3,$result4,$result5,$result6,$result7,$result8,$result9,$result10,$result11,$result12,$group_result);
				return $all_results;

	}

	public function check_payroll_period($payroll_period)
		{
			$this->db->select('complete_from,complete_to');
			$this->db->from('payroll_period');
			$this->db->where('id',$payroll_period);
			$query = $this->db->get();
			return $query->row();	
		}

	public function get_condition($option,$val)
	{
		$locc 	= substr_replace($option, "", -1);
		$location =  explode('-', $locc);
		$string_l="";
		foreach($location as $a)
            { 	 
            	$dd = $val.'="'.$a.'" or ';
                $string_l .= $dd;
            }
        $res_l = substr($string_l, 0, -4);
        $where_l = "(".$res_l.")";
        return $where_l;

	}


	public function working_schedule_fields($report){
		$this->db->select('*');
		$this->db->from('report_fields');
		$this->db->join('crystal_report_time','crystal_report_time.report_time_id=report_fields.report_time_id');
		$this->db->where('report_id',$report);
		$query = $this->db->get();
		return $query->result();
	}

	public function get_month_selected($date_from,$date_to)
	{
			$month_from = date('m', strtotime($date_from));
			$month_to = date('m', strtotime($date_to));

			$yr_from = date('y', strtotime($date_from));
			$yr_to = date('y', strtotime($date_to));

			$yr_count = 0;
			for($y = $yr_from; $y <= $yr_to;$y++)
			{
				$yr_count = $yr_count+1;
			}

			if($yr_count == 1)
			{
				$month_selected = $this->get_monthselected($month_from,$month_to);
				

			}
			else if($yr_count==2)
			{
				$month_1 = $this->get_monthselected($month_from,12);
				$month_2 = $this->get_monthselected(1,$month_to);
				$month_selected = $month_1.$month_2; 
			}

			else
			{
				$month_1 = $this->get_monthselected($month_from,12);
				$month_2 = $this->get_monthselected(1,$month_to);
				$month_3 = $this->get_monthselected(1,12);

				$month_selected = $month_1.$month_2.$month_3;
			}
			
			return $month_selected;
	}

	public function get_monthselected($month_from,$month_to)
	{
			$month_selected = '';
			for($m=$month_from;$m <=$month_to;$m++)
				{
					if($m=='1'){ $mm = 1; } else if($m=='2'){ $mm = 2; } else if($m=='3'){ $mm = 3; }
					else if($m=='4'){ $mm = 4; } else if($m=='5'){ $mm = 5; } else if($m=='7'){ $mm = 7; }
					else if($m=='8'){ $mm = 8; } else if($m=='9'){ $mm = 9; } else { $mm= $m; }

					if($mm==1 || $mm==2 || $mm==3 || $mm==4 || $mm==5 || $mm==6 || $mm==7|| $mm==8 || $mm==9)
					{
						$month = '0'.$mm;
					}
					else
					{
						$month = $mm;
					}

					$dd = $month."-";
	                $month_selected .= $dd;
				}

				return $month_selected; 
	}
}
