...
;		}
	...
	public function get_all(){
		...
		$this->db->join('section h', 'g.department_id = h.department_id');
		$this->db->join('classification i', 'c.classification = i.classification_id');
		if(!empty($this->input->post('department')) AND $this->input->post('department')!='all'){
		$this->db->where('g.department_id',$this->input->post('department'));
		}
		if(!empty($this->input->post('section')) AND $this->input->post('section') !='all'){
		$this->db->where('h.section_id',$this->input->post('section'));
		}
		if(!empty($this->input->post('classification')) AND $this->input->post('classification') !='all'){
		$this->db->where('i.classification_id',$this->input->post('classification'));
		}
		if(!empty($this->input->post('employee'))){
		$this->db->where('c.employee_id',$this->input->post('employee'));
		}



				if(!empty($this->input->post('from')) AND $this->input->post('to') != 'all'){
			...
			if(!empty($this->input->post('c')) !='all'){
			$this->db->where('b.company_id',$this->input->post('c'));
		}
	...
	public function get_employees($c){
		$this->db->select('*');
		$this->db->from('employee_info');
...
public function get_department($c){
	$this->db->select('*');
	...
	public function get_section($c){
	$this->db->select('*');
	$this->db->from('section');
	$this->db->where('department_id',$c);
	$query = $this->db->get();
    return $query->result();
}
	public function get_classification($c){
	$this->db->select('*');
