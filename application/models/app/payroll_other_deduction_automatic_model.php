...
class Payroll_other_deduction_automatic_model extends CI_Model{
...
//GETTING THE LIST OF EMPLOYEE ENROLL TO THIS PAYTYPE================================================
	...
	public function list_enroll_emp_paytypes($od_id,$cut_off_id,$pay_type,$company_id){
		...
		$query = $this->db->get("other_deduction_automatic");
...
//GETTING ADDITION TYPE LIST==============================================================
	...
	public function deduction_type_list($company_id){
		...
		$query = $this->db->get("other_deduction_type_automatic");
		return $query->result();
	}
	
	public function getting_deduction($od_id){
		$this->db->select('other_deduction_type,other_deduction_code');
		$this->db->where(array(
			'id'		=>	$od_id
		));		
	...
	public function get_deduction_result_add($company_id){
...
public function flash_deduct_update($id){
		$this->db->where(array(
			'id'		=>	$id
		));		
		$query = $this->db->get("other_deduction_type");
...
public function flash_deduct_updates($od_id){
		$this->db->where(array(
			'id'		=>	$od_id
		));		
		$query = $this->db->get("other_deduction_type");
...
//IS AUTOMATIC TO ZERO==============================================================================
	public function delete_auto($od_id){
		$this->db->where(array(
			'id'		=>	$od_id
		));		
		$query = $this->db->get("other_deduction_type");
...
public function is_automatic_to_zero($company_id,$id){
		...
		$this->db->update("other_deduction_type_automatic",$this->data);	
	}
...
public function automatic_deduction_saves($cutoff,$company_id,$id,$effectivity_date,$pay_type,$is_automatic){ 
		 $cut_off = $cutoff;
		 $company = $company_id;
		 $other_deduction_id = $id;
		...
		$this->db->insert('other_deduction_type_automatic',$this->data);

	}

public function get_deduction_result_edit($od_id,$company_id){
...
public function paytypeList_deduction_selected($pay_type){
		// do not include "Daily" , do not create payroll period for daily
		// $this->db->where('pay_type_id !=', '1');
		// $this->db->where('pay_type_id !=', '4');
		$this->db->order_by('pay_type_id','asc');
...
public function viewDetails_model($cutoff,$pay_type,$date_effective,$od_id,$company_id,$id)
	{
		$this->db->select('*');
		$this->db->from('other_deduction_type_automatic');
...
public function automatic_deduction_saves_e($cutoff,$company_id,$id,$od_id,$effectivity_date,$pay_type,$is_automatic){ 
		...
		$this->db->update("other_deduction_automatic",$this->data);
	}

//UPDATE SET AUTOMATIC WITHOUT EMPLOYEE==================================================================================
public function automatic_deduction_saves_ne($cutoff,$company_id,$id,$od_id,$effectivity_date,$pay_type,$is_automatic){ 
		//$id	= $this->input->post('addition_id');
		$pay_type_id = $pay_type;
...
public function getodid($o_d_id,$company_id){
...
function save_deduction_enroll_automatic(){  
		...
		if($other_deduction_type->num_rows()>0){
			...
			$this->db->select("employee_id,other_deduction_id,pay_type,cutoff,open_entry"); 
		$other_deduction_automatic = $this->db->get("other_deduction_automatic");
		if($other_deduction_automatic->num_rows()>0){
			foreach($other_deduction_automatic->result() as $data){
		...
		if($employee_info->num_rows()>0){
			foreach($employee_info->result() as $data){
			 ...
			 foreach($arr_od as $rs_od){
				...
				if($this->input->post('cod'.$rs_od.'-'.$data->employee_id)==1){
				
					
						if(isset($rs_od) && $this->input->post('od'.$rs_od.'-'.$data->employee_id)>0 || isset($rs_od) && $this->input->post('pf'.$rs_od.'-'.$data->employee_id)>0){

								if($arr_exist >0){
							$this->db->delete("other_deduction_automatic", array('other_deduction_id' => $rs_od, 'employee_id'=>$data->employee_id,'pay_type' => $this->input->post('pay_type'),'cutoff' => $this->input->post('cutoff')));

							$amount = $this->input->post('od'.$rs_od.'-'.$data->employee_id);
							$payroll_formulas = $this->input->post('pf'.$rs_od.'-'.$data->employee_id);
	 						$this->data = array();
							$this->data = array(
								...
								$this->db->insert('other_deduction_automatic',$this->data);
							}

						}else{ 
								$this->db->delete("other_deduction_automatic", array('other_deduction_id' => $rs_od, 'employee_id'=>$data->employee_id));
...
public function check_exist($employee_id,$pay_type,$od_id,$company_id,$cut_off){ 
...
public function insertImport($data)//model for insertImport
    {
        $query = $this->db->insert('other_deduction_automatic', $data); 
		if(!$query)
		{
		   return False;
		}
