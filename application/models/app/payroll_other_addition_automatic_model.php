...
class Payroll_other_addition_automatic_model extends CI_Model{
	...
	public function checkEmp($employee_id,$company_id){
		$query=$this->db->query("SELECT employee_id,company_id,name_lname_first FROM masterlist WHERE employee_id='".$employee_id."' and company_id='".$company_id."'");
		
		return $query->row();
	}


	public function check_other_addition($other_addition_id){

		$query=$this->db->query("SELECT other_addition_type FROM other_addition_type WHERE id='".$other_addition_id."' ");
		return $query->row();
	}
	

	public function check_oa_formula($formula_id,$company_id){
		$query=$this->db->query("SELECT formula_description FROM payroll_formulas WHERE formula_id='".$formula_id."' AND formula_tier='15' AND InActive='0' ");
		return $query->row();
	}


	public function insert_auto_addition_template($auto_addition_template){
		$this->db->insert('other_addition_automatic',$auto_addition_template);
