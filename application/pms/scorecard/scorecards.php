...
<div  style=" text-align:center"><h3 class="box-title" style="cursor: pointer;" onclick="get_employee_info_status('<?php echo $employee_id; ?>');"><strong style="color:#3c8dbc;"><?php echo $name; ?></strong></h3>
      ...
      <button id='update_form' class="btn btn-primary btn-block" onclick="dl(<?php if(!empty($employeeid)){echo $employeeid;}?>,'<?php if(!empty($doc_no->doc_no)){ echo $doc_no->doc_no; } ?>','<?php if(!empty($ref_eval->ref)){ echo $ref_eval->ref; }else{ echo '';} ?>')" >Ready for Evaluation</button>
	 <br>
	 <br>
	<div id="slim-scroll">
