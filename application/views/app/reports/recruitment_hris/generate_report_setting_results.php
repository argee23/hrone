  ...
  ?>
    ...
    <?php foreach($crystal_report as $cc){
          ...
          if($code_type=='S2' AND $cc_field=='numbering')
              {
                $get_numbering = $this->report_recruitments_hris_model->get_numbering_application_status($d->company_id,$d->id,'numbering');
                if(empty($get_numbering)){ $data = "no setup yet"; } else{ $data = $get_numbering; }
              } 
          else if($code_type=='S2' AND $cc_field=='include_comp')
              {
                $get_comp = $this->report_recruitments_hris_model->get_numbering_application_status($d->company_id,$d->id,'include_in_computation_job_vacancy');
    ...
    ?>
      <td>
        <?php 
            ...
            else if($code_type=='S15' AND $cc_field=='data')
            {
               if($data=='disable_cancellation')
               {
                  echo "Disable Cancellation";
               }
               else if($data=='within_the_day'){
                 echo "Cancellation within the day of pending status";
               }
               else if($data=='pending_status')
               {
