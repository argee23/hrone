...
<!-- FILE MAINTENACE LIST ================================================================================================= -->
      ...
      <div id="philhealth_print">

        <div class="col-md-12">
          <div class="box box-primary">
            <div class="panel panel-info">
            <div class="panel-heading"><strong>PHILHEALTH PREMIUM CONTRIBUTION</strong><a href="<?php echo base_url(); ?>app/payroll_file_maintenance/" type="button" class="btn btn-primary btn-xs pull-right" title="Enable account" ><i class="fa fa-arrow-circle-left"></i> Select a company</a></div>
            ...
            <div class="col-md-12">
              ...
              <div><h5><strong><?php echo $philhealth_company->company_name;?></strong>

              <a onclick="philhealth_print('<?php echo $company; ?>')" type="button" class="pull-right" data-toggle="tooltip" data-placement="left" title="View/Print"><i class="fa fa-print fa-2x text-danger pull-right"></i></a>

              <a href="<?php echo site_url('app/payroll_file_maintenance/philhealth_copy_standard/'. $company.''); ?>" type="button" class="pull-right" data-toggle="tooltip" data-placement="left" title="Copy standard table to current year <?php echo date('Y', strtotime(date("Y-m-d"))); ?>"><i class="fa fa-files-o fa-2x text-success pull-right"></i></a>
...
<!-- //=======================START -->
<div class="col-md-12">
<div id="ph_per_emp" class="collapse">
  <div class="col-md-12 bg-success">
     <div class="panel-heading"><strong> >> Set It Up By Location</strong></div>
  <form class="form-horizontal" method="post" action="<?php echo base_url()?>app/payroll_file_maintenance/save_sched_by_loc_ph/<?php echo $company;?>" >
    <?php
    if(!empty($compLoc)){
...
<?php
if(!empty($ph_ded_sched)){
  if($ph_ded_sched->single_field=="yes"){//  employee sss deduction schedule differs even if the same company
    $ph_sched="per_employee";
  }else{// all employee have the same sss deduction schedule per company
    $ph_sched="per_company";
  }
}else{//pag walang setup : default is all employee have the same sss deduction schedule per company
  $ph_sched="per_company";
}

if($ph_sched=="per_company"){
?>


<label for="company">Add/Edit Deduction (cut-off)<a onclick="philhealth_cutoff_edit('<?php echo $company; ?>')" type="button" class="pull-right" data-toggle="tooltip" data-placement="right" title="edit"><i class="fa fa-pencil-square-o fa-lg text-warning pull-right"></i></a></label>
...
<button data-toggle="collapse" data-target="#ph_per_emp" class="btn btn-danger"><i class="fa fa-pencil-square-o fa-lg"></i>Set Deduction Schedule [per employee]</button>
...
?>
              ...
              <label for="company">Effective year</label>
                ...
                <?php
                  foreach($philhealth_date as $date){
     ...
     <div class="col-md-6">
              ...
              <label for="company">Select Pay Type</label>
                <select class="form-control" name="pay_type_id" id="pay_type_id" onchange="applyFilter_by_paytype(); display_cutoff();">
                     ...
                     <?php
                           foreach($paytype_philhealth as $pay_type){
                              $pt='';
            ...
            <div class="col-md-12">
            <div class="form-group">            
            <div id="philhealth_table">

            <table class="table table-bordered table-striped">
            <thead>
              <tr>
                <th style="width:15%; text-align: center;" >PAY TYPE</th>
  ...
  <script>
//START OF PHILHEALTH CONTRIBUTION EDIT===========================================================================================
function display_cutoff()
    ...
    xmlhttp.onreadystatechange=function()
     ...
     xmlhttp.open("GET","<?php echo base_url();?>app/payroll_file_maintenance/display_philhealth_cutoff_by_paytype/"+pay_type_id+"/"+company+"/"+date,false);
    xmlhttp.send();
...
function applyFilter_by_paytype()
    ...
    xmlhttp.onreadystatechange=function()
     ...
     xmlhttp.open("GET","<?php echo base_url();?>app/payroll_file_maintenance/philhealth_table_result_paytype/"+pay_type_id+"/"+company+"/"+date,false);
    xmlhttp.send();

    }

function philhealth_cutoff_edit_save()
 {
            ...
            { 
               /*       if (window.XMLHttpRequest)
                ...
                xmlhttp.onreadystatechange=function()
                 ...
                 xmlhttp.open("GET","<?php echo base_url();?>app/payroll_file_maintenance/philhealth_cutoff_edit_save/"+cutoff+"/"+company_id+"/"+pay_type_id,true);
...
//pay tpe option result
  ...
  //END OF PHILHEALTH CONTRIBUTION EDIT===========================================================================================
    ...
    function philhealth_table_add(company_id,pay_type_id)
    {            
        ...
        xmlhttp.open("GET","<?php echo base_url();?>app/payroll_file_maintenance/getPhilhealthTable_add/"+company_id+"/"+pay_type_id,true);
        xmlhttp.send();
    }


    function philhealth_table_edit(val)
    {            
        if (window.XMLHttpRequest)
          {
          xmlhttp=new XMLHttpRequest();
        ...
        xmlhttp.open("GET","<?php echo base_url();?>app/payroll_file_maintenance/getPhilhealthTable_edit/"+val,true);
        xmlhttp.send();
    }

    function philhealth_print(val)
    {   
        var date      = document.getElementById("date").value;
        ...
        xmlhttp.onreadystatechange=function()
          ...
          if (xmlhttp.readyState==4 && xmlhttp.status==200)
            ...
            document.getElementById("philhealth_print").innerHTML=xmlhttp.responseText;
            }
          }
        xmlhttp.open("GET","<?php echo base_url();?>app/payroll_file_maintenance/philhealth_print_view/"+val+"/"+date,false);
        xmlhttp.send();
    }
    function philhealth_cutoff_edit(val)
    {   
        ...
        xmlhttp.open("GET","<?php echo base_url();?>app/payroll_file_maintenance/philhealth_cutoff_edit/"+val,true);
        xmlhttp.send();
    }



function disabled_ee_and_er(str){
  
  if(str==110){
    document.getElementById('ee_share').disabled = true;
    document.getElementById('er_share').disabled = true;
  }else{
    document.getElementById('ee_share').disabled = false;
    document.getElementById('er_share').disabled = false;
  }
