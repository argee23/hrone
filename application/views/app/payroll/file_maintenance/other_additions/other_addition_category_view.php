...
<div class="panel panel-success" >
  <div class="panel-heading table-responsive " >
        ...
        <?php 
         ...
         ?>
      </strong><strong>(OTHER ADDITIONS CATEGORY)</strong>

      

       <a onclick="add_new_addition_category(<?php echo $company_id;?>)" type="button" class="btn btn-xs btn-default pull-right " data-toggle="tooltip" data-placement="left" title="Add Category"><i class="fa fa-plus text-danger"></i>Add New Category</a>
                  ...
                  <tr <?php if($categories->IsDisabled == 1){echo 'style="color:#999;""';}else{echo 'class="text-success"';} ?>>
                    ...
                    <?php if($categories->IsDisabled == 0){ ?>
                        ...
                        <?php
                         ...
                         echo $edit = '<i '.$to_edit_enabled.' class="hidden" data-toggle="tooltip" data-placement="left" title="Edit '.$categories->category.'" onclick="addition_category_table_edit('.$categories->id.')"></i>'; 
                        ?>

                        <a href="<?php echo base_url()?>app/payroll_file_maintenance_additions/deactivate_other_addition/<?php echo $categories->id;?>"><i <?php echo $this->session->userdata('check_leave_type_todisable_icon'); ?> class="hidden"  data-toggle="tooltip" data-placement="left" title="Click to Disable <?php echo $categories->category?>'" onclick="return confirm('Are you sure you want to disable <?php echo $categories->category?> category?')"></i></a>

                        <a href="<?php echo base_url()?>app/payroll_file_maintenance_additions/delete_category/<?php echo $categories->id;?>/<?php echo $categories->company_id;?>"><i class="fa fa-remove fa-lg text-success pull-right"  data-toggle="tooltip" data-placement="left" title="Click to Delete <?php echo $categories->category?>'" onclick="return confirm('Are you sure you want to delete <?php echo $categories->company_id.' : '.$categories->category?> category?')"></i></a>
                    ...
                    <?php 
                        ...
                        <a href="<?php echo base_url()?>app/payroll_file_maintenance_additions/activate_other_addition/<?php echo $categories->id;?>"><i <?php echo $this->session->userdata('check_leave_type_toenable_icon'); ?> class="hidden" data-toggle="tooltip" data-placement="left" title="Click to Enable <?php echo $categories->category?>'" onclick="return confirm('Are you sure you want to enable <?php echo $categories->category?> category?')"></i></a>
