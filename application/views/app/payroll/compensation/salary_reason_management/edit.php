<form method="post" action="<?php echo base_url()?>app/payroll_compensation/edit_save/<?php echo $this->uri->segment("4"); ?>" >
...
<div class="col-md-12">
  ...
  <tbody>
      <tr>
        <td align="center" >
          <input type="text" class="form-control" name="reason_title" id="reason_title" placeholder="Reason title" onchange="return trim(this)" value="<?php echo $reason_info->reason_title; ?>"  required>
        </td>
        <td align="center" >
          <textarea name="reason_description" rows="5" cols="50" class="form-control" placeholder="Reason description" onchange="return trim(this)" required><?php echo $reason_info->reason_description; ?></textarea>
        </td>
        <td>Active</td>
        <td>
        <button type="submit" class="btn btn-danger btn-xs pull-right" ><i class="fa fa-check fa-lg"  data-toggle="tooltip" data-placement="right" title="Save" ></i></button>
        </td>
      </tr>
    <?php $check = false;
    foreach($company_reason as $company){ ?>
     <input type="hidden" class="form-control" name="company_id" id="company_id" placeholder="Reason title" onchange="return trim(this)" value="<?php echo $reason_info->company_id; ?>"  required>
    <tr>
