...
//============GET PHILHEATLH GROSS
	if($philhealth_formula==""){// no formula for PHILHEALTH gross yet
				...
				/*every 1stcutoff*/	if($philhealth_deduct_on=="1"){
										...
										if($is_philhealth_netbasic_basis_fixed=="yes"){
						if($active_salary_rate=="3"){											
												$philhealth_formula_value=($mysalary_amount*$mysalary_no_of_days_yearly)/12;
												$net_basic_means="(salary amount[$mysalary_amount] * no of days a year[$mysalary_no_of_days_yearly])/12";
						}else{
												$philhealth_formula_value=$net_basic_value*2;
										...
										$philhealth_formula_text="1st Cut Off Deduction will base on net basic only : ".number_format($philhealth_formula_value,$payslip_decimal_place)." <i class='fa fa-arrow-left'></i>($net_basic_means)";
										$philhealth_gross=0;
