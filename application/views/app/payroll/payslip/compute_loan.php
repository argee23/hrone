<?php
$pause_loan_text="";
$loan=$this->payroll_generate_payslip_model->get_employee_loan($employee_id,$pay_period,$pay_period_from,$pay_period_to);
if(!empty($loan)){
		$total_loan="";
		$loan_text="";
...
//check if has additional loan
$myAdditionalLoan=$this->payroll_generate_payslip_model->CheckAdditionalLoan($employee_id,$pay_period_from,$emp_loan_id);			
if(!empty($myAdditionalLoan)){
	$total_additional_loan=$myAdditionalLoan->additional_loans;
}else{
	$total_additional_loan="";
}
...
// ===================== end check history of deduction
			...
			//start add the additional loans
			$orig_loan_amt=$orig_loan_amt+$total_additional_loan;
			//end add the addtional loans
			$loan_amt=$myloan->loan_amt;
			$loan_amt=$loan_amt-$total_prev_system_deduction;
			if($total_additional_loan>0){
				$loanbal_basis_howto="(".$myloan->loan_amt."+$total_additional_loan)";
			}else{
				$loanbal_basis_howto="(".$myloan->loan_amt.")";
			}

			$as_of_loan_balance_basis=$myloan->loan_amt+$total_additional_loan."->".$loanbal_basis_howto;
...
if($selected_payroll_option=="post_all"){
	...
	if(($cut_off=="1")AND($first_cutoff_payslip_state=="posted")){
		...
		$save_each_loan_amortization = array(
			...
			'original_balance' => $as_of_loan_balance_basis,//$orig_loan_amt,
