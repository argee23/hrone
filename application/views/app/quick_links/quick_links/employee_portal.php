  <?php require_once(APPPATH.'views/employee_portal/header.php');?>
                  ...
                  <ul class="nav nav-pills nav-stacked">
                      <?php if(empty($get_modules)) { echo "<li><n class='text-danger'>No Module Found.</n></li>"; } else { 
                        foreach($get_modules as $m){
                          if($m->module=='Approval')
                          {
                            if ($this->session->userdata('is_form_approver')==true || $this->session->userdata('is_notification_approver')==true || $this->session->userdata('is_salary_manager')==true) {  $checker = true; } else{ $checker=false; }
                          }
                          else if($m->module=='My Subordinates')
                          {
                            if ($this->session->userdata('is_section_manager')==true) {  $checker = true; } else{ $checker=false; }
                          }
                          else if($m->module=='Reports')
                          {
                              if ($this->session->userdata('is_section_manager')==true || $this->session->userdata('is_form_approver')==true
                              || $this->session->userdata('is_notification_approver')==true || $this->session->userdata('is_salary_manager')==true) {  $checker = true; } else{ $checker=false; }
                          }
                          else
                          {
                            $checker = true; 
  ...
  <div class="col-md-9" style="padding-bottom: 50px;">
    ...
    <div class="modal fade" id="modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
     ...
     <?php require_once(APPPATH.'views/employee_portal/footer.php');?>                  