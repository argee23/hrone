<script>
    ...
    function eventClick(calEvent,eventt,employee_id)
    {
       ...
       if(color=='#5F9EA0')
       {
         ...
         var msg = 'This is viewing only.You can directly add new schedule.Do you want to add new schedule?';
                 var result = confirm(msg);
                 if(result == true)
                  { 
       ...
       else
       {
        ...
        $.ajax({
            ...
            'success': function (data) {
              ...
              else if(final == '"dtr_processed"')
              {
                 // alert('You are not allowed to remove plotted schedule if dtr is already processed.');
                  var msg = 'Do you want to replot schedule?';
                 var result = confirm(msg);
                 if(result == true)
                  { 
                      dayClick(eventt,employee_id);
            ...
            else if(final=='"dtr_locked"')
            {
               alert('You are not allowed to remove plotted schedule if dtr processing is locked.');
            }
            else if(final=='"fixed"')
            {
              var msg = 'This is Fixed Schedule viewing.Do you want to add new working schedule?';
                 var result = confirm(msg);
                 if(result == true)
                  { 
                      dayClick(eventt,employee_id);
      ...
      var company = document.getElementById('i_comp').value;
        ...
        $.ajax({
            ...
            'success': function (result) {
              ...
              {
                alert('You are not allowed to add/repplot plotted schedule if dtr processing is locked.');
              }
              else if(final == '"with_approved_changesched"')
              {
            ...
            if (xmlhttp.readyState==4 && xmlhttp.status==200)
              { 
                ...
                $('#sm_calendar').fullCalendar({
                    ...
                    height: 500,
                    fixedWeekCount: false,
                    events: '<?php echo base_url();?>employee_portal/section_mngr_management/get_schedule_by_group/' + group,
                });
              }
            }
    ...
    function view_other_individual_plotting()
    {
        $('#original_individual_plotting').hide();
         $('#other_individual_plotting').show();

        
    }

     function cancel_other_individual_plotting()
    {
        $('#original_individual_plotting').show();
         $('#other_individual_plotting').hide();
