          ...
          <div class="box box-primary">
            ...
            <div class="btn-group-vertical btn-block">
                ...
                <?php foreach($companyList as $company){?>
                  <a onclick="view_comploc_discipline('<?php echo $company->company_id; ?>')" type='button' class='btn btn-default btn-flat'><p class='text-left'><strong><?php echo $company->company_name; ?></strong></p></a>
                <?php } ?>
...
<script>
    ...
    {      
     ...
     // var location_id = document.getElementById('location_id').value;
      //alert(location_id); 
      window.open("<?php echo base_url();?>app/code_of_discipline/view_all_cod/"+company_id,true);
      window.send();
    ...
    {   
      ...
      window.open("<?php echo base_url();?>app/code_of_discipline/view_all_cod_disob/"+val+"/"+company_id,true);
      window.send();
    ...
    function view_comploc_discipline(company_id)
    {       
        if (window.XMLHttpRequest)
          {
          xmlhttp=new XMLHttpRequest();
                     ...
                     //Add New Modal
                                      ...
                                      $.ajax({
                                          ...
                                          success: function(response){
                                              ...
                                              if(response.type == 'exist'){
                                                 ...
                                                 $('#alertsuccess').html('Code of Discipline Title Already Exist!').fadeIn().delay(4000).fadeOut('slow');
                        ...
                        //DELETE 
                           $('#showdata').on('click','.item-delete', function(){
                            ...
                            //DISABLE 
                           $('#showdata').on('click','.item-disable', function(){
                                  var id = $(this).attr('data');
                                  $('#disableModal').modal('show');

                                  $('#btnDisable').unbind().click(function(){

                                      $.ajax({
                                          type: 'ajax',
                                          method: 'get',
                                          async: false,
                                          url: '<?php echo base_url()?>app/code_of_discipline/disabled_cod',
                                          data: {id: id},
                                          dataType: 'json',
                                          success: function(response){
                                              showallcod();
                                            if(response.success){
                                             $('#alertsuccess').removeClass('alert-success');
                                             $('#alertsuccess').addClass('alert-success');
                                             $('#alertsuccess').html('Code of Discipline Disable Successfully').fadeIn().delay(4000).fadeOut('slow');
                                             $('div.dataTables_filter input').focus();
                                              }else{
                                                alert('Error');
                                              }
                             ...
                             //DISABLE 
                           $('#showdata').on('click','.item-enable', function(){
                                  var id = $(this).attr('data');
                                  $('#enableModal').modal('show');

                                  $('#btnEnable').unbind().click(function(){

                                      $.ajax({
                                          type: 'ajax',
                                          method: 'get',
                                          async: false,
                                          url: '<?php echo base_url()?>app/code_of_discipline/enabled_cod',
                                          data: {id: id},
                                          dataType: 'json',
                                          success: function(response){
                                              showallcod();
                                            if(response.success){
                                             $('#alertsuccess').removeClass('alert-success');
                                             $('#alertsuccess').addClass('alert-success');
                                             $('#alertsuccess').html('Code of Discipline Enable Successfully').fadeIn().delay(4000).fadeOut('slow');
                                             $('div.dataTables_filter input').focus();
                                              }else{
                                                alert('Error');
                                              }
                              ...
                              $('#codnumbering').change(function(){
                                  ...
                                  if(cod_num != '' || cod_num != 0){
                                      $.ajax({
                                          ...
                                          method: 'POST',
                                          data: {cod_num: cod_num, company_id: company_id},
                                         // dataType: 'json',
                                          success:function(data){
                                                               ...
                                                               html += '<tr >'+
                                                                    ...
                                                                    /* '<input type="hidden" class="form-control" name="loc_id" id="loc_id" value="'+data[i].location_id+'">'+*/
                                                                        ...
                                                                        '<td>'+
                                                                            ...
                                                                            '<a style="margin-left:2px; margin-top:2px;" href="javascript:;" class="pull-right"  data-toggle="tooltip" title="Delete '+data[i].numbering+' Code of Discipline"><?php echo '<i class="fa fa-'.$system_defined_icons->icon_delete.' fa-'.$system_defined_icons->icon_size.'x" style="color:'.$system_defined_icons->icon_disable_color.';" "></i>';?></a>'+

                                                                             '<a style="margin-left:2px; margin-top:2px;" href="javascript:;" class="item-edit pull-right" data-toggle="tooltip" title="Edit '+data[i].numbering+' Code of Discipline"><?php echo '<i class="fa fa-'.$system_defined_icons->icon_edit.' fa-'.$system_defined_icons->icon_size.'x" style="color:'.$system_defined_icons->icon_disable_color.';" "></i>';?></a>'+

                                                                              '<a style="margin-left:2px; margin-top:2px;" href="javascript:;" class="item-enable pull-right" data="'+data[i].cod_id+'"  data-toggle="tooltip" title="Click to Enable '+data[i].numbering+' Code of Discipline"><?php echo '<i class="fa fa-'.$system_defined_icons->icon_enable.' fa-'.$system_defined_icons->icon_size.'x" style="color:'.$system_defined_icons->icon_enable_color.';" "></i>';?></a>'+
                                                                        '</td>'+
                                                                    '</tr>';
                                                              }else{
                                                               html += '<tr >'+
                                                                        ...
                                                                        '<td>'+
                                                                            ...
                                                                            '<a style="margin-left:2px; margin-top:2px;" href="javascript:;" class="item-delete pull-right" data="'+data[i].cod_id+'" data-toggle="tooltip" title="Delete '+data[i].numbering+' Code of Discipline"><?php echo '<i class="fa fa-'.$system_defined_icons->icon_delete.' fa-'.$system_defined_icons->icon_size.'x" style="color:'.$system_defined_icons->icon_delete_color.';" "></i>';?></a>'+
                                                                             ...
                                                                             '<a style="margin-left:2px; margin-top:2px;" href="javascript:;" class="item-edit pull-right" onclick="disremoveedit('+data[i].cod_id+');" data="'+data[i].cod_id+'" data-toggle="tooltip" title="Edit '+data[i].numbering+' Code of Discipline"><?php echo '<i class="fa fa-'.$system_defined_icons->icon_edit.' fa-'.$system_defined_icons->icon_size.'x" style="color:'.$system_defined_icons->icon_edit_color.';" "></i>';?></a>'+
                                                                              ...
                                                                              '<a style="margin-left:2px; margin-top:2px;" href="javascript:;" class="item-disable pull-right" data="'+data[i].cod_id+'" data-toggle="tooltip" title="Click to Disable '+data[i].numbering+' Code of Discipline"><?php echo '<i class="fa fa-'.$system_defined_icons->icon_disable.' fa-'.$system_defined_icons->icon_size.'x" style="color:'.$system_defined_icons->icon_disable_color.';" "></i>';?></a>'+
                                                                        '</td>'+
                                                                    '</tr>';
                                                              }
        ...
        xmlhttp.open("GET","<?php echo base_url();?>app/code_of_discipline/view_comploc_discipline/"+company_id,true);
        xmlhttp.send();
    }
...
function view_disob(val)
    {     
                              ...
                              //alert(id);
                               ...
                               $.ajax({
                                  ...
                                  success: function(data){
                                     ...
                                     $('input[name=newdischktitles]').val(data.check_title);
                              ...
                              $('#disobedience').change(function(){
                                  ...
                                  if(disobedience != '' || disobedience != 0){
                                      $.ajax({
                                          ...
                                          method: 'POST',
                                          data: {disobedience: disobedience, company_id: company_id, cod_id: cod_id, cod_disob_id: cod_disob_id},
                                         // dataType: 'json',
                                          success:function(data){
                              ...
                              $('#numdis').change(function(){
                                  ...
                                  if(disobedience != '' || disobedience != 0){
                                      $.ajax({
                                          ...
                                          method: 'POST',
                                          data: {numdis: numdis, company_id: company_id, cod_id: cod_id, cod_disob_id: cod_disob_id},
                                         // dataType: 'json',
                                          success:function(data){
                          ...
                          $('#show_punishment').on('click','.item-edit-punish', function(){
                              ...
                              $.ajax({
                                  ...
                                  success: function(data){
                                     ...
                                     $('textarea[name=suspun]').val('');
                                     $('textarea[name=suspun]').attr('disabled',true);
                                     $('select[name=suspun]').attr('readonly',false);
                                    }else{
                                     $('textarea[name=suspun]').val(data.punish);
                                     $('textarea[name=suspun]').attr('disabled',false);
                                     $('select[name=suspun]').attr('readonly',true);
                                     }
        ...
        xmlhttp.open("GET","<?php echo base_url();?>app/code_of_discipline/view_disobedience/"+company+"/"+cod_id,true);
        xmlhttp.send();
    }

function parasaothers(val){
  alert(val);
  var data = document.getElementById("suspun"+val).value;
  if(data == 0 || data == ""){
    $('#btnSaveob').attr("disabled", true);
  }else if(data == 1){
   $('#suspun'+val).attr('name', 'suspun[]');
   $('#suspun'+val).attr("readonly",false);
   $('#suspuntext'+val).attr("disabled", true);
   ...
   $('#suspun'+val).removeAttr('name');
   $('#suspun'+val).attr("readonly",true);
   $('#suspuntext'+val).attr("disabled", false);
   $('#btnSaveob'+val).removeAttr('disabled');
  }
...
function gototitle(){
   ...
   var chk_title = tinyMCE.get('title').getContent({format : 'text'});
   var chk_desc = tinyMCE.get('description').getContent({format : 'text'});
  
     document.getElementById("newtitle").value = titles;
     document.getElementById("newdesc").value = descriptions;
     document.getElementById("check_title").value = chk_title;
     document.getElementById("check_desc").value = chk_desc;
}  

function gototitledis(){

   var titles = window.parent.tinymce.get('distitle').getContent();
   var chk_title = tinyMCE.get('distitle').getContent({format : 'text'});
  // alert(titles);
     document.getElementById("newdistitle").value = titles;
      document.getElementById("newdischktitle").value = chk_title;
...
function gototitledised(){
   ...
   var chk_title = tinyMCE.get('distitles').getContent({format : 'text'});
   //alert(titles);
     document.getElementById("newdistitles").value = titles;
     document.getElementById("newdischktitles").value = chk_title;
