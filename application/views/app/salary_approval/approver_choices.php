 <?php 
  /*
    ...
    -----------------------------------
    */
    $add_salary_app_choice=$this->session->userdata('add_salary_app_choice');
    $del_salary_app_choice=$this->session->userdata('del_salary_app_choice');
...
<br><ol class="breadcrumb"><h4 class="text-danger" style="font-weight: bold;"><i class="fa fa-bars"></i>Set up Approvers Choices</h4></ol>
 ...
 <?php  
     if($add_salary_app_choice=="hidden "){
          ...
          <table id="approver_choices" class="col-md-12 table table-hover table-striped">
                ...
                <?php foreach ($approver_list as $approver) { ?>
                 <tr>
                    ...
                    <td><a class='<?php echo $del_salary_app_choice;?> fa fa-<?php echo $system_defined_icons->icon_delete;?>' style='cursor:pointer;  color:<?php echo $system_defined_icons->icon_delete_color;?>' aria-hidden='true' data-toggle='tooltip' title='Click to delete record!'  onclick='deleteApprover(<?php echo $approver->employee_id?>)'></a></td>
                 </tr>
                <?php } ?>
                </tbody>
       </table>
