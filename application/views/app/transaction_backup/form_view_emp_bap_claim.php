...
<title><?php echo $empCompany->company_name; ?></title><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
...
<div  id="printableArea"> <!-- this is important -->
  ...
  <tr>
    <th colspan="4" style="text-align: center">BURIAL ASSISTANCE PROGRAM (BAP) CLAIM FORM
  ...
  <tbody style="font-size: 10px;">
      <tr>
    <td colspan="4" style="text-align: center;"><hr><p class="text-primary">-- Claimants Info --</p></td>
  </tr>
  <tr>
 ...
 <tr>
    <td width="20%"><p class="text-primary">ADDRESS:</p></td>  
    <td width="">
   address
    </td>    
   ...
   <tr>
    <td><p class="text-primary">CONTACT INFO:</p></td>
    <td>contact info</td>
  ...
  <tr>
    <td><p class="text-primary">Name of deceased:</p></td>
    <td><?php echo $file_doc->deceased_name;?></td>
    <td><p class="text-primary">Date Of Birth:</p></td>
    <td><?php 

    $dbd_m=substr($file_doc->deceased_bdate, 5,2);
    $dbd_d=substr($file_doc->deceased_bdate, 8,2);
    $dbd_y=substr($file_doc->deceased_bdate, 0,4);

    echo date("F", mktime(0, 0, 0, $dbd_m, 10))." ". $dbd_d.", ". $dbd_y;
    ?></td>
  </tr>
  <tr>
    <td><p class="text-primary">Relationship to claimant:</p></td>
    <td><?php echo $file_doc->relation_to_claimant;?></td>
    <td><p class="text-primary">Age:</p></td>
    <td><?php 
    $cd=date('Y-m-d');
$date1 = new DateTime($file_doc->deceased_bdate);
...
echo $diff->y . " years, " . $diff->m." months, ".$diff->d." days "
  ...
  <tr>
    <td><p class="text-primary">Place of funeral / Wake:</p></td>
    <td><?php echo $file_doc->funeral_wake_place;?></td>
    <td><p class="text-primary">Place of interment / Burial:</p></td>
    <td><?php echo $file_doc->interment_burial_place;?></td>
  </tr>
  <tr>
    <td><p class="text-primary">Date of Death:</p></td>
    <td><?php 

    $dd_m=substr($file_doc->death_date, 5,2);
    $dd_d=substr($file_doc->death_date, 8,2);
    $dd_y=substr($file_doc->death_date, 0,4);

    echo date("F", mktime(0, 0, 0, $dd_m, 10))." ". $dd_d.", ". $dd_y;
    ?></td>
    <td><p class="text-primary">Date of Burial:</p></td>
    <td><?php 

    $bd_m=substr($file_doc->burial_date, 5,2);
    $bd_d=substr($file_doc->burial_date, 8,2);
    $bd_y=substr($file_doc->burial_date, 0,4);

    echo date("F", mktime(0, 0, 0, $bd_m, 10))." ". $bd_d.", ". $bd_y;
    ?></td>
  </tr>
...
$get_all_app=$this->transaction_employees_model->get_all_app($file_doc->doc_no,$table_name);
