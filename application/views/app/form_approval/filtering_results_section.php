...
<table class="table table-bordered table-striped">
              <thead>
                <tr  class="info">
                   <th colspan="8"><n class="text-danger" style="font-weight:bold;">Department: </n><n><b><?php echo  $d->dept_name?></b></n>
                   <n class='<?php echo $clear_all_from_approver;?> pull-right'><i class='btn btn glyphicon glyphicon-remove' onclick="delete_app_all('<?php echo $company?>','<?php echo $d->department_id?>','<?php echo $identification?>','department')"></i>[Delete Approvers in this department]</n></th>
                 </tr>
              </thead>  
              <tbody>
              <?php $section = $this->form_approval_model->filtering_sections($d->department_id,'department_id','section',$sectionss);
                ...
                ?>
                <tr class="danger">
                <td colspan="8" style="width:20%;" class='text-info'><i><b> <n class='text-danger'>Section:</n> <?php echo $s->section_name?></b></i><a class='<?php echo $clear_all_from_approver;?> pull-right' style='cursor: pointer;' onclick="delete_app_all('<?php echo $company?>','<?php echo $s->section_id?>','<?php echo $identification?>','section')">[Delete Approvers in this section]</a></td>
                </tr>
                <?php $subsection = $this->form_approval_model->filtering($s->section_id,'section_id','subsection'); 
                if(empty($subsection)){ echo "<tr class='text-success'><td colspan='2'></td><td colspan='3' class='text-danger'>No Subsection/s yet.</td></tr>"; } else{
                foreach($subsection as $ss){ ?>
                <tr class='success'>
                
                  <td colspan="2"><n class='text-success'> Subsection : <?php echo $ss->subsection_name?></n></td>
                ...
                <tr>
                  ...
                  <td><i class='<?php echo $clear_all_from_approver;?> btn btn glyphicon glyphicon-remove' onclick="delete_applocclassdivsec_all('<?php echo $company?>','<?php echo $a->id?>','<?php echo $identification?>','id','<?php echo $location?>','<?php echo $classification?>','<?php echo $division?>','<?php echo $department?>','<?php echo $section?>')"></i></td>
                </tr>
              <?php } } } } }} }?>
