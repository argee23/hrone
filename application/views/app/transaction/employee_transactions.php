...
<option value="transaction_forms_settings" <?php if($tran_fm_set_settings=="hidden "){echo 'disabled title="Not Allowed.Check User Rights." ';}else{}?> >Transaction Form Settings</option>
<option value="page_dl_ul_tran_record" <?php if($tran_fm_du_app_form=="hidden "){echo 'disabled title="Not Allowed.Check User Rights." ';}else{}?> >Download/Upload Transaction Records</option>
<option value="blocked_leave_dates" <?php if($tran_fm_block_dates_leave=="hidden "){echo 'disabled title="Not Allowed.Check User Rights." ';}else{}?> >Block dates for leave application</option>
<option value="atro_members" <?php if($tran_fm_set_ot_policy=="hidden "){echo 'disabled title="Not Allowed.Check User Rights." ';}else{}?> >Enrol Employee ATRO Policy</option>
<option value="trip_ticket" <?php if($tran_fm_mng_trip_ticket=="hidden "){echo 'disabled title="Not Allowed.Check User Rights." ';}else{}?> >Manage Trip Ticket</option> 
    ...
    <!-- <option value="page_mass_tran_enc">Mass Transaction Encoding</option>double check and turn me on next time -->
...
if($tran_fm_approve=="hidden "){
   echo '<i  style="color:#ccc;" class="fa fa-'.$system_defined_icons->icon_info.' fa-'.$system_defined_icons->icon_size.'x" title="Not Allowed.Check Access Rights"></i>';
}else{
...
if($tran_fm_del=="hidden "){
   echo '<i  style="color:#ccc;" class="fa fa-'.$system_defined_icons->icon_info.' fa-'.$system_defined_icons->icon_size.'x" title="Not Allowed.Check Access Rights"></i>';
     ...
     <!--    <td> -->
          <!-- //===============================transfer approval of transactions  -->  
          <?php 
...
<!--         </td> -->
 ...
 <!--        </td> -->
        <td>
          <!-- //===============================error transactions (no assigned approvers) -->  
          <?php 
...
function download_template()
   ...
   function next()
    ...
    function  tripticket_home()
    {
      ...
      //added by mi / 2018-09-10
      function get_setting_action(company)
      {
        document.getElementById('settings_options').value="";
        document.getElementById('settings_transactions').value="";
      }

      function update_settings(transaction)
      {
        var company_id = document.getElementById('settings_company').value;
        var settings = document.getElementById('settings_options').value;

        if(transaction=='' || company_id=='' || settings=='')
        {
          alert("Please fill up all fields to continue");
        }
        else
        {
          ...
          xmlhttp.onreadystatechange=function()
            ...
            if (xmlhttp.readyState==4 && xmlhttp.status==200)
              ...
              document.getElementById("settings_action").innerHTML=xmlhttp.responseText;
              $("#settings_action_table").DataTable({
                           lengthMenu: [[-1,10, 25, 50, 100], ["All",10, 25, 50, 100]]             
                          });
              }
            }
          xmlhttp.open("GET","<?php echo base_url();?>app/transaction_employees/update_transaction_forms_settings/"+transaction+"/"+company_id+"/"+settings,false);
          xmlhttp.send();
        }
        
      }

      function updatesettings(transaction)
      {
        var company_id = document.getElementById('settings_company').value;
        var settings = document.getElementById('settings_options').value;
        ...
        if(transaction==2)
        {
                ...
                if(company_id=='all') 
                 {
                   $("#settings_leave").show(); 
                   $("#settings_leavem").show();
                    alert("Please choose one company to continue. In leave transaction , setting is by company leave type.");
                    document.getElementById('settings_transactions').value="";
                 }
                 else
                 {      $("#settings_leave").show();
                        ...
                        xmlhttp.onreadystatechange=function()
                          ...
                          if (xmlhttp.readyState==4 && xmlhttp.status==200)
                            ...
                            document.getElementById("settings_leave").innerHTML=xmlhttp.responseText;
                            }
                          }
                        xmlhttp.open("GET","<?php echo base_url();?>app/transaction_employees/get_transaction_leave_type/"+transaction+"/"+company_id+"/"+settings,false);
                        xmlhttp.send();
                 }
        ...
        { 
           $("#settings_leave").hide();
           $("#settings_leavem").hide();
           save_update_settings(transaction,company_id,settings);
        }
       
      }
      function save_update_settings(transaction,company_id,settings)
      {
             if(transaction=='' || company_id=='' || settings=='')
                {
                  alert("Please fill up all fields to continue");
      ...
      function update_settings_leave()
      {
           var company_id = document.getElementById('settings_company').value;
           var settings = document.getElementById('settings_options').value;
           var transaction = document.getElementById('settings_transactions').value;
           var leave = document.getElementById('settings_leave').value;
           
            if(transaction=='' || company_id=='' || settings=='' || leave=='')
                {
                  alert("Please fill up all fields to continue");
                }
                else
                {
                  ...
                  xmlhttp.open("GET","<?php echo base_url();?>app/transaction_employees/update_transaction_forms_settings_forleave/"+transaction+"/"+company_id+"/"+settings+"/"+leave,false);
                  xmlhttp.send();
                }

      }

      function for_disbled_checkbox(i,value)
      {
        if(value=='' || value=='no_settings')
        {
      ...
      function save_setting_action()
      {
        document.getElementById('company_data').value="";
        document.getElementById('settings_value').value="";
        document.getElementById('settings_checked').value="";

        var comp =   document.getElementsByClassName('company_class');
        var count = document.getElementById('counts_data').value;


        for (i=0;i<count;i++)
        {
            comp[i].checked = true;
            comp_data = comp[i].value;
            var selected_company = document.getElementById('company_data').value;
            var res = selected_company +=comp_data + "-";
            document.getElementById('company_data').value=res;


            settings_data = document.getElementById('settingdata'+i).value;

            if(settings_data=='' || settings_data=='no_settings')
            {
              settings_data='no_settings';
            }
            var selected_data = document.getElementById('settings_value').value;
            var res_data = selected_data +=settings_data + "-";
            document.getElementById('settings_value').value=res_data;
            ...
            var selected_checked = document.getElementById('settings_checked').value;
            var res_checked = selected_checked +=check_val + "-";
            document.getElementById('settings_checked').value=res_checked;
         
        }
        save_transaction_form_settings(count,res,res_checked,res_data);
      }

      function save_transaction_form_settings(count,res,res_checked,res_data)
      { 
          var option = document.getElementById('settings_options').value;
          var transaction = document.getElementById('settings_transactions').value;
          var company_option = document.getElementById('settings_company').value;
          ...
          xmlhttp.open("GET","<?php echo base_url();?>app/transaction_employees/save_transaction_form_settings/"+count+"/"+res+"/"+res_checked+"/"+res_data+"/"+option+"/"+transaction+"/"+company_option,false);
          xmlhttp.send();
      }

      function get_transactions_setting(setting)
      {
          var company = document.getElementById('settings_company').value;
         if (window.XMLHttpRequest)
            {
            xmlhttp=new XMLHttpRequest();
            }
          ...
          xmlhttp.onreadystatechange=function()
            ...
            if (xmlhttp.readyState==4 && xmlhttp.status==200)
              {
                document.getElementById("settings_transactions").innerHTML=xmlhttp.responseText;
              }
            }
          xmlhttp.open("GET","<?php echo base_url();?>app/transaction_employees/get_transactions_setting/"+company+"/"+setting,false);
          xmlhttp.send();
      }

      function save_setting_action_forleave(transaction,company,setting,leave)
      { 
       
        var count = document.getElementById('counts_data_forleave').value;
        for (i=0;i<count;i++)
        {
            settings_data = document.getElementById('settingdataleave'+i).value;
            if(settings_data=='' || settings_data=='no_settings')
            {
              settings_data='no_settings';
            }
            var selected_data = document.getElementById('settings_valueleave').value;
            var res_data = selected_data +=settings_data + "-";
            document.getElementById('settings_valueleave').value=res_data;


            
            ltype = document.getElementById('settingdataltype'+i).value;
            if(ltype=='' || ltype=='no_settings')
            {
              ltype='no_settings';
            }
            var selected_datal = document.getElementById('settings_valueleavetype').value;
            var res_datal = selected_datal +=ltype + "-";
            document.getElementById('settings_valueleavetype').value=res_datal;

        }
        var datas  =  document.getElementById('settings_valueleave').value;
        var leavedata = document.getElementById('settings_valueleavetype').value;
        if(transaction=='' || company=='' || setting=='' || datas=='' || count=='' || leavedata=='')
        {
          alert("Please fill up all fields to continue");
        }
        else
        {
            ...
            xmlhttp.open("GET","<?php echo base_url();?>app/transaction_employees/save_transaction_form_settings_forleave/"+transaction+"/"+company+"/"+setting+"/"+datas+"/"+count+"/"+leavedata+"/"+leave,false);
            xmlhttp.send();
        }
