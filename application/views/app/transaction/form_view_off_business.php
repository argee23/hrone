...
<div  id="printableArea"> <!-- this is important -->
 ...
 $cur_form= $this->uri->segment('6');
  ...
  <tr>
    <th colspan="4" style="text-align: center">OFFICIAL BUSINESS FORM 
  ...
  <tr>
    <td><p class="text-primary">COMPANY NAME:</p></td>
    <td><?php echo $file_doc->company_name;?></td>
    <td><p class="text-primary">COMPANY ADDRESS:</p></td>
    <td>  <?php echo $file_doc->company_address?> </td> 
  </tr>

  <tr>
    <td ><p class="text-primary">INCLUSIVE DATE</p></td>
    <td ><?php echo $file_doc->from_date." to ".$file_doc->to_date; ?></td>
    <td ><p class="text-primary">INCLUSIVE TIME:</p></td>
    <td> <?php echo $file_doc->from_time."  to  ".$file_doc->to_time; ?> </td> 
  </tr>

  <tr>
    <td ><p class="text-primary">WILL RETURN:</p></td>
    <td>  <?php if($file_doc->will_return=="1"){echo "Yes";}else{ echo "No";} ?> </td>
    <td><p class="text-primary">WITH MEAL</p></td>
    <td><?php if($file_doc->with_meal=="1"){echo "Yes";}else{ echo "No";} ?></td>
  </tr>

   <tr>
    <td><p class="text-primary">OB DATES:</p></td> 
    <td>
        <?php
          $ob_dates = $this->transaction_employees_model->get_ob_dates($file_doc->doc_no);
          $i=1;
          $obdates = '';
          foreach($ob_dates as $o)
          {
            if(count($ob_dates) == $i)
              { $obdates.=$o->the_date; } else{ $obdates.=$o->the_date." , "; } 
            $i++;
          }
          echo $obdates;
        ?>
