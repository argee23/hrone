     ...
     <canvas id="myChart" width="400" height="100"></canvas>
            ...
            <table  class="table table-hover table-responsive table-bordered">
              <thead>
                ...
                <tr><td><?php echo $section; ?></td><td>
                  
  <?php foreach($display_count as $qwe){
                            if($qwe->grading_type == 1){
                                       $i = $qwe->ranking; $n = $qwe->section_id; 
                             }else{ 
                                      $i = $qwe->ranking; $n = $qwe->section_id;
                            }
                             ...
                             $count = $this->report_analytics_model_pms->count4_section($qwe->ranking,$n);
                               $total = 0;  
        ...
        labels: [<?php
         foreach($display_section as $display_section){
          if($grading_type->grading_type == 1){ 
            echo "'".$display_section->score.' '.$display_section->score_equivalent."', "; 
        }else{ 
          echo "'".$display_section->ranking.' '.$display_section->score_equivalent."', ";
        }} 
        ...
        datasets: [{
            ...
            data: [   <?php 
                           ...
                           foreach($display_count as $qwe){
                            if($qwe->grading_type == 1){ $i = $qwe->ranking; $n = $qwe->section_id; }else{ $i = $qwe->ranking; $n = $qwe->section_id;}
                             $ci = & get_instance();
                              $ci->load->model("app/report_analytics_model_pms");
                              $count = $ci->report_analytics_model_pms->count_section($i,$n);
                            if(!empty($count)){
                                $counts  = $count;
                            }else{
                                $counts = 0;
