            ...
            <div class="box-body">
              ...
              <div class="row">
                  ...
                  <div class="form-group">
                    ...
                    <select required="" name="company" class="selectpicker" id="company" data-live-search="true" data-width="100%" onchange="get_depart(this.value);" data-style="btn-default" >
                ...
                <div class="col-md-3">
                  <div class="form-group">
                    <label for="">employee</label>
                      ...
                      <select name="employee"  id="sel_depart" class="selectpicker" data-live-search="true" data-width="100%" data-style="btn-default" >
                ...
                <div class="col-md-2">
                  ...
                  <select name="from" id="chart_type" data-width="100%" class="selectpicker">
                              <?php foreach($get_appraisal_schedule as $to){ ?>

    <option value="<?php echo $to->appraisal_period_type_dates; ?>"><?php echo date("d-M-Y", strtotime($to->appraisal_period_type_dates)); ?></option>
        <?php } ?>
                  </select>
                </div>
                       <div class="col-md-2">
                  <label for="">To</label>
                  <select name="to"  data-width="100%" class="selectpicker">
                              <?php foreach($get_appraisal_schedule as $from){ ?>

    <option value="<?php echo $from->appraisal_period_type_dates; ?>"><?php echo date("d-M-Y", strtotime($from->appraisal_period_type_dates)); ?></option>
        <?php } ?>
                  </select>
                </div>
                <!-- col-md-4 -->
...
<span class="pull-right">Page rendered in <strong>{elapsed_time}</strong> seconds. <b>Version</b> 1.0</span>
    ...
    if ($("#btnFilter")[0].checkValidity()){
        $.ajax({
            url: "<?php echo base_url();?>app/report_analytics_pms/filtered_employee/",
 ...
 {$('#sel_depart').selectpicker('refresh').empty();
     ...
     jQuery.ajax({
        type: "POST",
        url: "<?php echo base_url();?>app/report_analytics_pms/get_employee/",
        dataType: 'json',
        data: {"id" : element},
        success: function(res) {
               $.each(res,function(index,data){
                    $('#sel_depart').append('<option value="'+data['employee_id']+'|'+data['fullname']+'">'+data['fullname']+'</option>');
