...
<ol class="breadcrumb">
<h4 class="text-danger" style="font-weight: bold;"><i class="fa fa-bars"></i>Evaluators
...
<div class="col-md-8">
    <div class="form-group">
     ...
     <select class="form-control" id="select" name="id" onchange="get_selected_evaluators(this.value)" >
        <?php if ($row->creators_type != ''){?>
        <option selected="" value="<?php echo $row->id; ?>"> <?php echo $row->creators_type; ?>  </option>
      <?php }else{ ?>
        <option selected=""> Select Evaluators </option>
      <?php  } ?>
...
<script type="text/javascript">
  ...
  get_selected_evaluators('<?php echo $get_selected_evaluators->creators_type ?>');
</script>
