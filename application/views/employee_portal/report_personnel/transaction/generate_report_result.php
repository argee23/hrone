                              ...
                              <?php 
                                ...
                                else
                                {
                                    ...
                                    if($t_table_name=='employee_leave')
                                    {
                                              ...
                                              { 
                                                  $leave_type = $this->reports_personnel_transactions_model->leave_type($cc->leave_type_id); 
                                                  if(!empty($leave_type)) { echo $leave_type; }
                                              }
                                              else if($field_title =='leave_dates')
                                              {
                                                 $leave_dates = $this->reports_personnel_transactions_model->leave_dates($cc->doc_no);
                                                 $i=1;
                                    ...
                                    {       
                                            ...
                                            {
                                                $request_list = $this->reports_personnel_transactions_model->request_list($cc->request_list);
                                                echo $request_list;
                                            } 
                                            else
                                            {
                                    ...
                                    else if($t_table_name=='emp_change_sched')
                                    {
                                        ...
                                        {
                                                $schedule_dates = $this->reports_personnel_transactions_model->schedule_dates($cc->doc_no);
                                                 $i=1;
                                    ...
                                    else if($t_table_name=='emp_grocery_items_loan')
                                    {
                                       if($field_title=='payroll_period')
                                        {
                                            $payroll_period = $this->reports_personnel_transactions_model->payroll_period($cc->payroll_period);
                                    ...
                                    else if($t_table_name=='emp_atro')
                                    {
                                      ...
                                      {
                                          $attendance = $this->reports_personnel_transactions_model->get_employee_attendance($cc->employee_id,$cc->atro_date);
                                    ...
                                    else if($t_table_name=='emp_bap_claim')
                                    {
                                      ...
                                      {
                                        $relation_to_claimant = $this->reports_personnel_transactions_model->system_param($cc->relation_to_claimant);
                                            if(empty($relation_to_claimant)){ echo ""; } else { echo $relation_to_claimant; }
                                      }
                                      else if($field_title=='deceased_religion')
                                      {
                                        $deceased_religion = $this->reports_personnel_transactions_model->system_param($cc->deceased_religion);
                                    ...
                                    else if($t_table_name=='emp_payroll_complaint')
                                    {
                                        ...
                                        {
                                          $complaint_type = $this->reports_personnel_transactions_model->complaint_type($cc->complaint_type); 
                                    ...
                                    else if($t_table_name=='emp_official_business')
                                    {
                                        ...
                                        {
                                              $ob_dates = $this->reports_personnel_transactions_model->ob_dates($cc->doc_no);
                                              $i=1;
                                    ...
                                    else if($t_table_name=='employee_leave_cancel')
                                    {
                                        ...
                                        if($field_title=='with_pay' || $field_title=='leave_dates' || $field_title=='leave_type')
                                            {
                                                $leave_details = $this->reports_personnel_transactions_model->leave_details($cc->cancelled_doc_no);
                                                foreach($leave_details as $leave)
                                                {
                                                    if($field_title=='with_pay'){ if($leave->with_pay==1){ echo "yes"; } else{ echo "no"; } } 
                                                          ...
                                                          $leave_type = $this->reports_personnel_transactions_model->leave_type($leave->leave_type_id); 
                                                          if(!empty($leave_type)) { echo $leave_type; } 
                                                    }
                                                    else
                                                    {
                                                        $leave_dates = $this->reports_personnel_transactions_model->leave_dates($cc->cancelled_doc_no);
                                                        $i=1;
                                                        foreach($leave_dates as $l)
                                                           {
                                    ...
                                    else if($t_table_name=='emp_loans')
                                    {
                                       if($field_title=='loan_type')
                                        {
                                            $loan_type = $this->reports_personnel_transactions_model->loan_type($cc->loan_type);
                                    ...
                                    else if($t_table_name=='emp_authority_to_deduct')
                                    {
                                        ...
                                        {
                                            $type_of_advance = $this->reports_personnel_transactions_model->type_of_advance($cc->type_of_advance);
                                            if(empty($type_of_advance)){ echo ""; } else { echo $type_of_advance; }
                                        }
                                        else if($field_title=='deduction_type')
                                        {
                                            $deduction_type = $this->reports_personnel_transactions_model->deduction_type($cc->deduction_type);
