<br> 
<form enctype="multipart/form-data" method="post" action="<?php echo base_url()?>employee_portal/reports_personnel_transactions/saveupdate_crystal_report" >
      ...
      <div class="box box-default"></div>
        <h4><center><n class="text-danger">Check the fields you want to view in printing reports</n></center></h4>
          <?php  $i=0; foreach($crystal_report_list as $row) {
             if($row->field_name=='approval_date' || $row->field_name=='approval_status' || $row->field_name=='approval_level' || $row->field_name=='approval_type') {} else{
           $checked=$this->reports_personnel_transactions_model->crystal_report_details_fields($crd->id,$row->id);
