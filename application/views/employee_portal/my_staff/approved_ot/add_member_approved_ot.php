...
<br><br>
  ...
  <form enctype="multipart/form-data" method="post" action="<?php echo base_url()?>employee_portal/my_staff_approved_ot/save_ot_approved_ot/<?php echo $id;?>/<?php echo $date;?>">
     

    <input type="hidden" id="check" value="<?php echo $check;?>">  
    <table style="width:96%;margin-left:2%;margin-top:20px;margin-bottom:40px;" border="1">
      <thead>
        <tr>  
      ...
      <tbody>
       <?php $i=1; foreach ($group_members as $p) {
         $stat = $this->my_staff_approved_ot_model->checker_member($p->section,$p->subsection,$p->location,$this->session->userdata('employee_id'));
         if($stat > 0) {

           $attendance = $this->my_staff_approved_ot_model->get_employee_attendance($p->employee_id,$date);
           $check_selected = $this->my_staff_approved_ot_model->check_selected($p->employee_id,$date);
           ...
           {
              $check_employee_filed_ot = $this->my_staff_approved_ot_model->check_employee_filed_ot($p->employee_id,$date);
              if(!empty($check_employee_filed_ot))
              {
                 $checker = "filed_employee";
              }
              else
              {
                  $checker = "";
       ...
       ?>
        ...
        <tr <?php if(!empty($checker)) { if($checker == 'exist_false') { echo "style='color:#FF6347'; "; } else if($checker=='filed_employee'){ echo "style='color:#4B0082'; ";  } else{ echo "style='color:red'; "; } } ?>>
            <td>
                  <?php echo $i;?>
            ...
            <td><input type="checkbox" name="employee_id<?php echo $i;?>" id="employee_id<?php echo $i;?>" class="form-control" 
                    <?php 
                          if(!empty($checker)) 
                          { 
                              if($checker == 'exist_false') { echo "disabled"; } 
                              if($checker == 'filed_employee') { echo "disabled"; } 
                              else{ echo "checked"; } } ?> >
            </td>
            <td><center><?php echo $p->employee_id;?><br>[<?php echo $p->first_name.' '.$p->last_name;?>]</center></td>
            <td>
              <?php if(!empty($checker)) { 
                        if($checker == 'exist_false') { echo $check_selected->hours; }
                        if($checker == 'filed_employee') 
                        { 
                            foreach($check_employee_filed_ot as $ott)
                            {
                              echo $ott->no_of_hours.' hour/s<a target="_blank" href="'.base_url().'employee_portal/employee_transactions/view/'.$ott->doc_no.'/emp_atro/HR008"> [view]</a>'.'<br>';
                            }
                        }
                        else 
                        { 
                            echo '<input type="text" id="total_hour'.$i.'" name="total_hour'.$i.'" name="" class="form-control" value="'.$check_selected->hours.'" onkeypress="return isNumberKey(this, event, '.$i.');">'; 
                        } 
                    }
                    else
                    { 
                        echo '<input type="text" id="total_hour'.$i.'" name="total_hour'.$i.'" class="form-control" value="0" onkeypress="return isNumberKey(this, event , '.$i.');">'; 
                    } 
              ?>
            ...
            <td>
                <?php
                        ...
                        $yy =  date("Y", strtotime($datef));
                                    ...
                                    $individual_schedules = $this->my_staff_approved_ot_model->get_individual_schedules($p->employee_id,$datef,$m,$yy);
                                    ...
                                    {
                                      $fixed_schedules = $this->my_staff_approved_ot_model->checker_if_fixed_sched($p->employee_id,$datef);
                                      ...
                                      {
                                          $group_schedules = $this->my_staff_approved_ot_model->checker_if_group_sched($p->employee_id,$datef);
            ...
            <td>
              <?php 
                    if(!empty($checker)) 
                    { 
                        if($checker == 'exist_false') 
                        { echo "Plotted by ".$check_selected->plotted_by.' with '.$check_selected->hours.' approved OT<br>[Group Name:'.$check_selected->group_name.']'; } 
                        else if($checker=='filed_employee')
                        { foreach($check_employee_filed_ot as $g) { if($g->entry_type=='manual_upload') { echo "Admin Manual Upload";  } else{ echo "Employee File";  } } } 
                        else { echo "With plotted approved OT "; }  
                    } 
                    else
                    { echo "No Approved OT yet"; } ?>
