...
<!--   <div class="wrimagecard wrimagecard-topimage">
            ...
            <div class="col-md-4">
               ...
               <div class="row">
                      ...
                      <div class="col-md-8 col-xs-12 col-sm-6 col-lg-8" >
                          <div class="container" >
                            <h2> <span id="full"><?php echo $name; ?></span>
                          ...
                          <hr>
                          <div class="col-sm-12 col-xs-12 tital " >Date Of Appraisal :<?php echo date_format(date_create($get_current_appraisal_schedule->appraisal_period_type_dates),'d-M-Y') ?> | EVALUATION PERIOD: <?php echo $get_current_appraisal_schedule->coverage ?></div>
                      </div>
                </div>
            </div>
            </div>
    ...
    <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
      ...
      <?php $form= array(); foreach($forms as $forms){ $grading_type = $forms->grading_type; ?>
...
<td>
  <table class="table"><?php $q = $this->pms_model->get_weight_and_description_admin($res1->cid,$res1->doc_no);
                    
      foreach($q as $q){?>

        <tr>
             <td ><?php echo $q->description?></td>
             <td width="60px"><?php echo $q->weight.'%' ?></td>
            <input type="hidden" class="c_cid" value="<?php echo $q->id ?>">
             <input type="hidden" id="c_employee_id" value="<?php echo $employee_id?>">
           <td width="60px" class="c_score">


            <?php if($get_computation->computation_type =='1'){ $w = $this->pms_model->get_score($forms->doc_no,$forms->fid,'admin', $q->id); 
            echo round($w['result']->score/$w['rows']);
          }else{
              $w = $this->pms_model->lastevalaution($forms->doc_no,$forms->fid,'admin', $q->id); echo $w->score;
            } 
           ...
           <td class="rate" width="60px">
                 ...
                 <?php  if($get_computation->computation_type =='1'){ $ws = $this->pms_model->get_score_rate($q->weight,$w['result']->score/$w['rows'],'admin'); echo $ws;
                          }else{
                            $ws = $this->pms_model->get_score_rate($q->weight,$w->score,'admin'); echo $ws;
                            }  ?>
  ...
  <tfoot><tr>
    <td></td>
  <td><u>Totals and Rating for Part <?php echo $forms->form_part; ?>:
  <div class="pull-right" > 
    <?php

  $sw =  $this->pms_model->get_score_equival($sq,$grading_type);
echo '<b><span style="padding-right:110px;">'.$qweea.'%</span>';  echo '<span style="padding-right:35px;">'.$sq.'</span>  </b>' ?>
</div>
        ...
        <?php   
              $form[] = array("form_title"=>$forms->form_title,"weight"=>$forms->weight,
                "score"=>$sq,
                "score_total"=>$forms->weight/100*$sq,
                "form_part"=>$forms->form_part);
           }?>
  ...
  <tfoot><tr><td colspan="4" ><u>Totals and Rating for Part <?php echo $forms->form_part; ?>: <?php echo '<b>'.$wr->total.'</b>' ?></u></td></tr>
     ...
     <table class="table table-bordered">
      ...
      <thead> <tr class='info'>
                               <th scope="col" colspan="5">Final Rating/Summary     </th>
      ...
      <td class="total"><?php echo $form[$key]['score_total']; $f +=  $form[$key]['score_total']; ?></td>
    </tr>
    <?php } ?>
   </tbody><tfoot>

      <td><u><b>Final rating table :</b> 
        <?php $sw =  $this->pms_model->get_score_equival($f,$grading_type,$company);
         echo $sw->score;  echo '<i>('.$sw->score_equivalent.')</i>'; ?></u>  
           <input type="hidden" id="score_equivalent" value="<?php echo $sw->score_equivalent;?>">    <input type="hidden" id="score_total" value="<?php echo $sw->ranking;?>"></td>
    </tfoot></table>
    </div>
      ...
      <ul class="nav navbar-nav">
        ...
        <li class="dropdownqwe " >
         ...
         <button onclick="next_appro(<?php echo $company; ?>)"  class="btn btn-success pull-right">Approve</button>
      </div>
    </div>
