...
<form id="form5" method="post">
				<input type="hidden" name="company" value="<?php echo $company; ?>">
					...
					<br>
						  ...
						  <?php  $data = array();  foreach($get_last_eval as $get_last_eval){ array_push($data,$get_last_eval->evaluators); }
						   if(in_array($this->session->userdata('employee_id'),$data)){ ?>
...
<?php } ?>
									 	...
									 	if(($f != 'false') OR ($f =='true')){
									 		 ...
									 		 ?>				
									 		  				  <ul class="nav navbar-nav">
																	<li class="dropdown"  id="4" onclick="criteria_form_admin(<?php echo $general_forms->fid ?>,'<?php  echo $this->uri->segment('4');  ?>','<?php echo $this->uri->segment('5'); ?>',<?php echo $general_forms->grading_type ?>,'<?php echo $this->session->userdata('employee_id'); ?>');",>
									 		 ...
									 		 <?php 
												...
												<ul class="nav navbar-nav">
														...
														<?php  if($first==1){?>
																	<li style="background-color: #e5e5e5" class="dropdown" id="4" onclick="criteria_form_admin(<?php echo $general_forms->fid ?>,'<?php  echo $this->uri->segment('4');  ?>','<?php echo  $this->uri->segment('5'); ?>','<?php echo $general_forms->grading_type ?>','<?php echo $this->session->userdata('employee_id'); ?>');",>
 ...
 if((e >= 1)	 && ($('input[name=l]').val() != $('input[name=eval]').val())){
     ...
     function qwe(id){
	     $.ajax({
            ...
            success: function(data){
           		if(data >= 1){
           			alert('Finish the current evaluating forms first ');
           		}else{
           			alert('You can now start evaluating the form');
           		}
            }
        }); 
