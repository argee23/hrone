    ...
    <div class="box-body">
      <div class="col-md-8"> <!-- Form Content -->
        <span class="dl-horizontal col-sm-7">
          ...
          <dt>Request</dt>
          <dd></dd>
         <?php  $request_form = $this->form_approver_model->get_request_form_list();
                $req = explode('-',$form->info->form->request_list);
              foreach($request_form as $rr) {?> 
            <dd><input type="checkbox" <?php foreach($req as $ee){ if($ee==$rr->id) { echo "checked"; } } ?> disabled> &nbsp;&nbsp;<?php echo $rr->title?></dd>
          <?php } ?>
         <?php if(empty($form->info->form->request_other)) {}else{ ?>  <dd><input type="checkbox" checked disabled> &nbsp;&nbsp;[Others][ <?php echo $form->info->form->request_other?> ] </dd><?php } ?>
        </span>
        <span class="dl-horizontal col-sm-5">
          
          <dt>Period of Employment</dt>
          <dd><?php echo date("F d, Y", strtotime($form->info->form->employment_period)); ?></dd>
          <dt>Date Filed</dt>
