		...
		$this->load->view('app/reports/pms/approval/approval',$data);
			...
			public function get_department(){
				...
				foreach($q as $e){
						...
						$s = $this->report_pms_model->get_classification($i);
									foreach($s as $e){
						...
						$q = $this->report_pms_model->get_employees($i);
				foreach($q as $e){
...
$stack = array();
			 ...
			 echo  "<table id='example' class='table display nowrap' style='width:100%;text-align:center;''><thead><tr><th>Name</th><th>Approver</th><th>status</th><th>Appraisal date</th><th>score</th><th>agreement</th></tr></thead><tbody>";
		foreach($get_all as $get_all){ 	
            ...
            if(!in_array($get_all->fullname ,$stack)){
           				 $stack[$get_all->employee_id] = $get_all->fullname;
           				 $emp[$get_all->employee_id] = $get_all->score;
           				 $num[$get_all->employee_id] = 1;
        ...
        echo '</tbody></table>';
        	if(!empty($stack)){	
           echo '<script>$("#qws").html("<h2>Average Score for '.date("F d, Y", strtotime($this->input->post('from'))).' to '.date("F d, Y", strtotime($this->input->post('to'))).'</h2>");</script>';
       	}	

        	foreach($stack as $key => $stack){
        		$q  = $emp[$key]/$num[$key];
         echo '<script>$.fn.dataTable.ext.errMode = "none";
					    $("#example").DataTable( {
        dom: "Bfrtip",
        buttons: [
         ...
         $("#qwes").append("<br/><b>'.$stack.':<b>&nbsp;'.round($q).'");</script>';
    	 }
