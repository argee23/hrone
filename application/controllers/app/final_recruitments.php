...
class Final_recruitments extends General{
	...
	{
		$insert = $this->final_recruitments_model->save_interview_request_first($date,$time,$message_final,$address_final,$id,$numbering,$app_id,$interviewer,$comment);
	}
	...
	public function save_updated_details($date,$time,$interviewer,$id,$company_id,$app_id,$employer_type,$numbering)
	{ 
		$this->data['company_id'] = $company_id;
		$this->data['app_id'] = $app_id;
		$this->data['employer_type'] = $employer_type;
		$this->data['interview_status']=$this->final_recruitments_model->company_interview_process($company_id);
		$this->data['interviewer']=$this->final_recruitments_model->company_interviewer($company_id);
		$this->data['action_']='updated';
		$this->session->set_flashdata('success_updated',"Success");
		$insert = $this->final_recruitments_model->save_updated_details($date,$time,$interviewer,$id,$app_id,$numbering);
		$this->load->view('app/final_recruitments/job_application/application_status_update_view',$this->data);
	}


	public function cancel_interview_request($id,$company_id,$app_id,$employer_type,$numbering)
	{ 
		$this->session->set_flashdata('message',"<div class='alert alert-success alert-dismissable'><i class='fa fa-check'></i><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Interview Request is successfully cancelled</div>");
		$cancelled = $this->final_recruitments_model->cancel_interview_request($id,$company_id,$app_id,$employer_type,$numbering);
	}
	...
	public function save_blocked_applicant_job_application($app_id,$employer_type)
	{
		...
		$action = $this->final_recruitments_model->save_for_interview_blocked($app_id);
		redirect(base_url().'app/final_recruitments/job_application_index/'."".$employer_type."/all/all",$this->data);
	}

	public function save_other_applicant_job_application($app_id,$employer_type,$stat_id)
	{
		$action = $this->final_recruitments_model->save_for_interview_other($app_id,$stat_id);
		redirect(base_url().'app/final_recruitments/job_application_index/'."".$employer_type."/all/all",$this->data);
	}

	public function update_application_status_other($job_id,$company_id,$employer_type,$app_id,$stat_id,$status,$company,$applicant_id)
	{
		...
		$this->data['company'] = $company;
		$this->data['app_info'] = $this->general_model->applicant_info($applicant_id,$job_id);
		$this->load->view('app/final_recruitments/job_application/application_status_update_other',$this->data);
	}


	public function save_manually_update($interview_id,$app_id,$response,$reason_final)
	{
		$update = $this->final_recruitments_model->save_manually_update($interview_id,$app_id,$response,$reason_final);
	}

	public function save_manually_update_finalreponse($interview_id,$app_id,$response,$reason_final)
	{
		$update = $this->final_recruitments_model->save_manually_update_finalreponse($interview_id,$app_id,$response,$reason_final);
	}

	public function view_application_history($app_id,$company_id,$employer_type,$job_id,$applicant_id)
	{
		$this->data['app_id'] = $app_id;
		$this->data['employer_type'] = $employer_type;
		$this->data['status']=$this->final_recruitments_model->get_company_applicaton_status($company_id);
		$this->data['interview_status']=$this->final_recruitments_model->company_interview_process($company_id);
		$this->data['app_info'] = $this->general_model->applicant_info($applicant_id,$job_id);
		$this->data['application_details'] = $this->final_recruitments_model->application_details($app_id);
		$this->load->view('app/final_recruitments/job_application/application_history',$this->data);

	}


	public function view_employee_referral($idd,$comp_id,$employer_type,$job_id,$applicant_id)
	{
		$this->data['job_id'] = $job_id;
		$this->data['applicant_id'] =  $applicant_id;
		// $this->data['app_id'] = $idd;
		// $this->data['employer_type'] = $employer_type;
		// $this->data['comp_id'] = $comp_id;
		$this->data['idd'] = $idd;
		$this->data['referral'] = $this->final_recruitments_model->get_referral_points($applicant_id,$job_id,$idd);
		$this->data['app_info'] = $this->general_model->applicant_info($applicant_id,$job_id);
		$this->load->view('app/final_recruitments/job_application/view_employee_referral',$this->data);
	}

	public function get_employees($company_id)
	{
		$employees =  $this->final_recruitments_model->get_employees($company_id);
		if(empty($employees))
		{
			echo "<option>No employe found.</option>";
		}
		else
		{
			foreach($employees as $emp)
			{
				echo "<option value='".$emp->employee_id."'>(".$emp->employee_id.")".$emp->fullname."</option>";
			}
		}
	}

	public function saveupdate_referral_status($id,$idd,$job_id,$applicant_id,$comment,$status,$employee)
	{	
		$this->data['job_id'] = $job_id;
		$this->data['applicant_id'] =  $applicant_id;
		$this->data['idd'] = $idd;
		$insert = $this->final_recruitments_model->saveupdate_referral_status($id,$idd,$job_id,$applicant_id,$comment,$status,$employee);
		$this->data['referral'] = $this->final_recruitments_model->get_referral_points($applicant_id,$job_id,$idd);
		$this->load->view('app/final_recruitments/job_application/employee_referral',$this->data);
	}
