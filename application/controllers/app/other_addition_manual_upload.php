...
class Other_addition_manual_upload extends General{

	function __construct(){
		parent::__construct();	
		$this->load->model("app/other_addition_manual_upload_model");
	...
	public function single_upload()
	{
    	...
    	if($option=='reset')
    	{
    		$delete_addition = $this->other_addition_manual_upload_model->delete_single_addition($group,$company,$payroll_period,$other_addition_id);
    	}
    	else
    	{
    		if(isset($_POST["import"]))
		    {
				...
				<center><h2>'.$action_result.' Other addition Employee Enrollment</h2></center>
					...
					?>
						<tr>
							...
							<td>
								<?php
										echo $employee_id;
										$resultemployee_companylist = $this->other_addition_manual_upload_model->check_employee_exist_in_company($employee_id,$company);
										...
										{
											$resultemployee_group = $this->other_addition_manual_upload_model->check_employee_exist_in_group($employee_id,$company,$group);
							...
							<td>
								<?php 
									...
									else
									{
										if($option=='add')
										{
											$insert_data = $this->other_addition_manual_upload_model->insertSingleaddition($employee_id,$company,$group,$payroll_period,$other_addition_id,$amount);
											if($error==false)
											{
	...
	public function mass_upload()
	{
    	...
    	if($option=='reset')
    	{
    		$delete_addition = $this->other_addition_manual_upload_model->delete_mass_addition($group,$company,$payroll_period);
    	}
    	else
    	{
    		if(isset($_POST["import"]))
				    ...
				    'font'  => array(
				     ...
				     for ($row = 2; $row <= $highestRow; $row++){
				     	...
				     	for($col = 0; $col < $colNumber; $col++){
							...
							elseif($col=="1")
							{
								$other_addition=$objPHPExcel->getActiveSheet()->getCellByColumnAndRow($col, $row)->getValue();
							}
							elseif($col=="2")
							{
								$amount=$objPHPExcel->getActiveSheet()->getCellByColumnAndRow($col, $row)->getValue();
					...
					?>
						<tr>
							...
							<td>
								<?php 
									echo $other_addition;
									$checker_addition = $this->other_addition_manual_upload_model->checker_addition($other_addition,$company);
									if($checker_addition===false)
									{
										echo "<br><n style='color:red';>Invalid Other addition Type</n>";
										$error = true;
									}
									else
									{
										echo "<br>[".$checker_addition."]";
									}
								?>
								...
								<?php 
									...
									else
									{
										if($option=='add')
										{
											if($error==false)
											{
												$insert_data = $this->other_addition_manual_upload_model->insertMassaddition($employee_id,$company,$group,$payroll_period,$amount,$other_addition);
												if($insert_data=='saved')
												{
													echo "<n style='color:green';><i>Saved</i></n>";
												}else{}
