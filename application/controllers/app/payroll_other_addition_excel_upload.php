...
class Payroll_other_addition_excel_upload extends General{

		function __construct(){
		parent::__construct();	
		$this->load->model("app/payroll_other_addition_excel_upload_model");
...
public function other_addition_excel_upload(){	 

		$company_id=$this->uri->segment('4');
		$this->data['company_classifications']= $this->general_model->get_company_classifications($company_id);
		$this->data['paytypeList_addition'] = $this->payroll_other_addition_excel_upload_model->paytypeList_addition();
		$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/view',$this->data);		
	}
...
public function single_upload(){	 
		...
		$this->data['company_classifications']= $this->general_model->get_company_classifications($company_id);
		$this->data['paytypeList_addition'] = $this->payroll_other_addition_excel_upload_model->paytypeList_addition();
		$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/selected_single',$this->data);		
	}
...
public function mass_upload(){	 
		...
		$this->data['company_classifications']= $this->general_model->get_company_classifications($company_id);
		$this->data['paytypeList_addition'] = $this->payroll_other_addition_excel_upload_model->paytypeList_addition();
		$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/selected_mass',$this->data);		
	}
...
//GET GROUP BY PAYTYPE=================================================================================
	public function by_group(){	
			...
			$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/by_group',$this->data);	
	...
	public function by_group_mass(){	
			...
			$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/by_group_mass',$this->data);	
		
	}

	public function select_addition(){	
			$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/select_addition',$this->data);	
...
//GET PAYROLL PERIOD BY GROUP PAYTYPE=================================================================================
	public function by_payroll_period(){	
		...
		$this->data['pay_per_addition'] = $this->payroll_other_addition_excel_upload_model->payroll_per_per_company_pay_type($company_id,$pay_type,$pay_type_group);//$company_id
		$this->data['addition_type'] = $this->payroll_other_addition_excel_upload_model->getAdditionTypes($company_id);
			$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/by_payroll_period',$this->data);	
	...
	public function by_payroll_period_mass(){	
		...
		$this->data['pay_per_addition'] = $this->payroll_other_addition_excel_upload_model->payroll_per_per_company_pay_type($company_id,$pay_type,$pay_type_group);//$company_id
		$this->data['addition_type'] = $this->payroll_other_addition_excel_upload_model->getAdditionTypes($company_id);
			$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/by_payroll_period_mass',$this->data);	
							     ...
							     if($col==1)
							          {
							           $value =$objPHPExcel->getActiveSheet()->getCellByColumnAndRow($col, $row)->getValue();
										...
										if($checkvalue === false){
							             ...
							             $result_emp_id_checker = $this->payroll_other_addition_excel_upload_model->empidnotexist($emp_id_checker);
		         ...
		         if($foundError==False){ // insert to employee_info table
		        	for ($row = 2; $row <= $highestRow; $row++){
	                 						...
	                 						$company_id = $this->input->post('company_id');
	               							 $dataexist = $this->payroll_other_addition_excel_upload_model->check_exist($employee_id,$pay_period,$oa_id,$company_id);
	               							 if($dataexist ==1){
	                	//ECHO "ALREADY EXIST IN THE DATABASE";
	                							$this->db->delete("other_addition_enrollment", array('other_addition_id' => $oa_id, 'employee_id'=>$employee_id,'payroll_period_id' => $this->input->post('pay_period')));
												}
											$dob = date($format = "Y-m-d", PHPExcel_Shared_Date::ExcelToPHP($rowData[0][4]));
	               							 $data = array(
	               								 ...
	               								 $insert = $this->payroll_other_addition_excel_upload_model->insertImport($data);
			            ...
			            if($insert){ //file name for successfully imported
								...
								$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/review_upload/template_header',$this->data);
								for ($row = 2; $row <= $highestRow; $row++){
									...
									$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/review_upload/template_content',$this->data);
								}
								$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/review_upload/template_footer',$this->data);
			...
			// end of else license purchase
					...
					$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/review_upload/template_header',$this->data);
						for ($row = 2; $row <= $highestRow; $row++){
							...
							$resultemployee_companylist = $this->payroll_other_addition_excel_upload_model->employee_company_checker_model($employee_companylist,$company,$pt_id);
							      ...
							      if($col==2)
									 ...
									 if($col == 0 ){
							   		...
							   		if($col == 1 ){
		                     			 ...
		                     			 $oa_id_checker =$objPHPExcel->getActiveSheet()->getCellByColumnAndRow($col, $row)->getValue();
							             $result_oa_id_checker = $this->payroll_other_addition_excel_upload_model->oaidnotexist($oa_id_checker);
							             	if($result_oa_id_checker === false){
							              	$objPHPExcel->getActiveSheet()->
							                setCellValueByColumnAndRow($col, $row,  $oa_id_checker.' -> '.$foroaid);//doesn't exist
							               	$objPHPExcel->getActiveSheet()->getStyle($colrow)->applyFromArray($styleArray)->getAlignment()->setWrapText(true);
							                $foundError = True;
								              }
								           }			
		         ...
		         if($foundError==False){ // insert to employee_info table
		        	for ($row = 2; $row <= $highestRow; $row++){
			            ...
			            if($insert){ //file name for successfully imported
								...
								$this->data['count_data']=$highestRow - 1;
								$this->data['company']=$company;
								$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/review_mass_upload/template_header',$this->data);
								for ($row = 2; $row <= $highestRow; $row++){
									$this->data['col_0']= $objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0, $row)->getValue();
									$this->data['col_1']= $objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1, $row)->getValue();
									$this->data['col_2']= $objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2, $row)->getValue();
									...
									$this->data['col_7']= $objPHPExcel->getActiveSheet()->getCellByColumnAndRow(7, $row)->getValue();
									$this->data['col_8']= $objPHPExcel->getActiveSheet()->getCellByColumnAndRow(7, $row)->getValue();
									$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/review_mass_upload/template_content',$this->data);
								}
								$this->load->view('app/payroll/other_addition_emp_enrollment/manual_excel_upload/review_mass_upload/template_footer',$this->data);
							       $dt = $date_array = getdate();
							       $formated_date  = "payroll_emploan_enrolment_";
							       $formated_date .= $date_array['mon'];
							       $formated_date .= $date_array['mday'];
