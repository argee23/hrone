<?php
//==== individual upload: partner is time_manual_attendance_logs_validation_grp.php
		...
		if(!empty($check_confusing_logs)){
			...
			$initial_logs_month=substr($initial_time_in_date, 5,2);
				...
				$recheck_confusing_logs=$bio_database_type->query("SELECT top 1 $employee_id_field_name,$logs_field_name,$logs_type_field_name from `$file_table_name`   WHERE $logs_field_name BETWEEN #$initial_time_in_date# AND #$initial_time_in_date_to# "." $for_individual AND $logs_type_field_name='".$code_in."' 
				...
				if($payperiod_dtr_lock=="yes"){// if dtr or payroll is alerady locked.
							...
							$update_prev_date=$this->db->query("update `$attendance_table` set covered_date='".$suppose_cd."',entry_type='Manual Upload:ConfusingLogs:TimeInFallstoNextday' where employee_id='".$selected_individual_employee_id."' AND time_in_date='".$initial_time_in_date."'");
